...
${outputFormat}`;
   ...
   * 獲取 rule 對應的溫度參數
   */
  static getTemperature(parsedQuery: ParsedQuery, defaultTemperature: number = 0.7): number {
   ...
   */
  static getMaxTokens(parsedQuery: ParsedQuery, defaultMaxTokens: number = 2000): number {
  ...
  static async executeRuleSearchTools(parsedQuery: ParsedQuery): Promise<string | null> {
    ...
    try {
      ...
      const activeSearchTools = parsedQuery.ruleConfig.searchTools.filter(tool => tool.isActive);
...
export class CustomerServiceManager {
  ...
  static async markConversationAsRead(id: string): Promise<boolean> {
    try {
      const response = await fetch(`${this.baseUrl}/api/widget/conversations/${id}/read`, {
        method: 'POST',
      });
