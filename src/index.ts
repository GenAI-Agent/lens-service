...
class LensServiceWidget {
  ...
  async init(config: ServiceModulerConfig): Promise<void> {
    ...
    const telegramConfig =
      config.telegram && config.telegram.botToken && config.telegram.chatId
        ? config.telegram
        : undefined;
   ...
   */
  private async callAzureOpenAIVision(
    message: string,
    imageBase64: string
  ): Promise<string> {
  ...
  private handleOpen(): void {
    ...
    this.conversationState = {
      sessionId: `sm_${Date.now()}_${Math.random()
        .toString(36)
        .substring(2, 15)}`,
      messages: [],
    };
