...
如果不需要任何工具，返回：
{
  "tools": [],
  "reason": "這是一般對話，不需要搜尋"
}`;try{const i=await this.openAI.chatCompletion([{role:"system",content:o},{role:"user",content:e}],.3,500),r=JSON.parse(i);return console.log("Tool selection reason:",r.reason),r.tools||[]}catch(i){return console.error("Failed to determine tools:",i),n.map(r=>r.id)}}async generateResponse(e,t,n){var l,c,p;const{systemPrompt:o,defaultReply:i}=await this.getSystemSettings();let r=((l=this.currentRule)==null?void 0:l.systemPrompt)||o;r+=`
...
搜尋結果：
${n}`:`
沒有找到相關的搜尋結果。`}`;const s=this.getRecentQA(t,2),a=[{role:"system",content:r}];s.length>0&&a.push({role:"system",content:`
--- 對話記憶（前 ${s.length} 次 QA）---
${s.join(`

`)}`}),a.push({role:"user",content:e});try{const h=await this.openAI.chatCompletion(a,((c=this.currentRule)==null?void 0:c.temperature)||.7,((p=this.currentRule)==null?void 0:p.maxTokens)||1e3),u=!h.includes("CANNOT_ANSWER");return u?{response:h.replace(/CANNOT_ANSWER/g,"").trim()||h,canAnswer:u}:{response:i,canAnswer:!1}}catch(h){return console.error("Failed to generate response:",h),{response:"抱歉，系統暫時無法處理您的請求。",canAnswer:!1}}}getRecentQA(e,t){const n=[];let o="";for(let i=e.length-1;i>=0&&n.length<t;i--){const r=e[i];r.role==="assistant"&&o?(n.unshift(`Q: ${o}
...
`).trim()}searchRelevantSections(e,t=5){const n=this.extract(),o=e.toLowerCase().split(/\s+/);return n.sections.map(r=>{let s=r.relevance;const a=r.heading.toLowerCase();o.forEach(c=>{a.includes(c)&&(s+=5)});const l=r.content.toLowerCase();return o.forEach(c=>{const p=(l.match(new RegExp(c,"g"))||[]).length;s+=p*2}),{heading:r.heading,content:r.content,score:s}}).sort((r,s)=>s.score-r.score).slice(0,t)}extractText(e){const t=e.cloneNode(!0);return t.querySelectorAll("script, style, noscript").forEach(n=>n.remove()),t.textContent||""}extractKeywords(e,t=20){const n=e.toLowerCase().replace(/[^\w\s\u4e00-\u9fa5]/g," ").split(/\s+/).filter(i=>i.length>1),o=new Map;for(const i of n)o.set(i,(o.get(i)||0)+1);return Array.from(o.entries()).sort((i,r)=>r[1]-i[1]).slice(0,t).map(([i])=>i)}generateFingerprint(e,t=64){const n=this.extractKeywords(e,50),o=new Array(t).fill(0);for(const i of n){const r=this.simpleHash(i,t);for(let s=0;s<t;s++)r[s]===1?o[s]++:o[s]--}return o.map(i=>i>0?1:0)}simpleHash(e,t){let n=0;for(let i=0;i<e.length;i++)n=(n<<5)-n+e.charCodeAt(i),n=n&n;const o=new Array(t).fill(0);for(let i=0;i<t;i++)o[i]=n>>i&1;return o}}class E{static setConfig(e){this.baseUrl="/api/lens"}static async query(e,t=[]){try{const o=await(await fetch(`${this.baseUrl}/query`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sql:e,params:t})})).json();if(!o.success)throw new Error(o.error||"Database query failed");return o.data}catch(n){throw console.error("Database query error:",n),n}}static async getSettings(){try{const t=await(await fetch(`${this.baseUrl}/settings`)).json();if(!t.success)throw new Error(t.error||"Failed to fetch settings");return t.data}catch(e){throw console.error("Get settings error:",e),e}}static async updateSetting(e,t){try{const o=await(await fetch(`${this.baseUrl}/settings/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({value:t})})).json();if(!o.success)throw new Error(o.error||"Failed to update setting");return o.data}catch(n){throw console.error("Update setting error:",n),n}}static async getAdminUsers(){try{const t=await(await fetch(`${this.baseUrl}/admin-users`)).json();if(!t.success)throw new Error(t.error||"Failed to fetch admin users");return t.data}catch(e){throw console.error("Get admin users error:",e),e}}static async createAdminUser(e,t,n){try{const i=await(await fetch(`${this.baseUrl}/admin-users`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:e,password:t,email:n})})).json();if(!i.success)throw new Error(i.error||"Failed to create admin user");return i.data}catch(o){throw console.error("Create admin user error:",o),o}}static async deleteAdminUser(e){try{const n=await(await fetch(`${this.baseUrl}/admin-users/${e}`,{method:"DELETE"})).json();if(!n.success)throw new Error(n.error||"Failed to delete admin user");return n.data}catch(t){throw console.error("Delete admin user error:",t),t}}static async login(e,t){try{const o=await(await fetch(`${this.baseUrl}/auth/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:e,password:t})})).json();if(!o.success)throw new Error(o.error||"Login failed");return o.data}catch(n){throw console.error("Login error:",n),n}}static async getManualIndexes(){return this.query(`
    ...
    `}showEditDialog(e,t,n=!1){return new Promise(o=>{const i=document.createElement("div");i.style.cssText=`
        ...
        align-items: center;
        z-index: 10000;
      `;const r=n?`<textarea id="edit-input" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; min-height: 120px; resize: vertical; font-family: inherit;">${t}</textarea>`:`<input type="text" id="edit-input" value="${t}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px;">`;i.innerHTML=`
      ...
      `,document.body.appendChild(i);const s=i.querySelector("#edit-input"),a=i.querySelector("#cancel-btn"),l=i.querySelector("#save-btn");s.focus(),s instanceof HTMLInputElement?s.select():s.setSelectionRange(0,s.value.length),a==null||a.addEventListener("click",()=>{document.body.removeChild(i),o(null)}),l==null||l.addEventListener("click",()=>{const c=s.value.trim();document.body.removeChild(i),o(c)}),s instanceof HTMLInputElement&&s.addEventListener("keydown",c=>{if(c.key==="Enter"){const p=s.value.trim();document.body.removeChild(i),o(p)}}),i.addEventListener("click",c=>{c.target===i&&(document.body.removeChild(i),o(null))})})}showConfirmDialog(e){return new Promise(t=>{var r,s;const n=document.createElement("div");n.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      `,n.appendChild(o),document.body.appendChild(n);const i=()=>{document.body.removeChild(n),t()};(r=o.querySelector("#alert-ok"))==null||r.addEventListener("click",i),n.addEventListener("click",s=>{s.target===n&&i()})})}bindEvents(){if(!this.container)return;const e=this.container.querySelector("#admin-login-form");if(e){e.addEventListener("submit",async h=>{h.preventDefault(),h.stopPropagation();const u=this.container.querySelector("#admin-username"),g=this.container.querySelector("#admin-password"),b=(u==null?void 0:u.value)||"",w=(g==null?void 0:g.value)||"";console.log("Login attempt with username:",b);try{const{DatabaseService:C}=await Promise.resolve().then(()=>$),M=await C.login(b,w);console.log("Login successful (database auth)"),this.isAuthenticated=!0,this.container.innerHTML=this.renderAdminUI(),await this.updatePageContent(),this.bindEvents()}catch(C){console.error("Login error:",C),this.showAlertDialog("登入時發生錯誤，請稍後再試").then(()=>{g.value="",g.focus()})}});const p=this.container.querySelector("#admin-username");p&&setTimeout(()=>{p.focus()},100)}setTimeout(()=>{const p=this.container.querySelectorAll(".nav-item");if(console.log("Binding nav items, found:",p.length),p.length===0&&this.isAuthenticated){console.warn("Nav items not found, retrying..."),setTimeout(()=>this.bindEvents(),100);return}p.forEach((h,u)=>{console.log(`Binding nav item ${u}:`,h.dataset.page);const g=h.cloneNode(!0);h.parentNode.replaceChild(g,h),g.addEventListener("click",async()=>{const b=g.dataset.page;console.log("Nav item clicked:",b),b&&(this.currentPage=b,this.container.innerHTML=this.renderAdminUI(),await this.updatePageContent(),this.bindEvents())})})},50);const t=this.container.querySelector("#admin-logout");t&&t.addEventListener("click",()=>{this.isAuthenticated=!1,this.container.innerHTML=this.renderLoginUI(),this.bindEvents()});const n=this.container.querySelector("#telegram-settings-form");n&&n.addEventListener("submit",async p=>{p.preventDefault(),p.stopPropagation();const h=this.container.querySelector("#telegram-enabled"),u=(h==null?void 0:h.checked)||!1;this.setTelegramEnabled(u),alert(`Telegram 通知已${u?"啟用":"停用"}`),await this.updatePageContent()});const o=this.container.querySelector("#change-password-form");o&&o.addEventListener("submit",async p=>{p.preventDefault(),p.stopPropagation();const h=this.container.querySelector("#new-password"),u=(h==null?void 0:h.value)||"";if(u.length<4){alert("密碼長度至少 4 個字元");return}f.saveAdminPassword(u),alert("密碼已更新"),await this.updatePageContent()});const i=this.container.querySelector("#ip-whitelist-form");i&&i.addEventListener("submit",async p=>{p.preventDefault(),p.stopPropagation();const h=this.container.querySelector("#ip-list"),g=((h==null?void 0:h.value)||"").split(`
`).map(b=>b.trim()).filter(b=>b.length>0);this.saveIPWhitelist(g),alert(`已更新 IP 白名單（${g.length} 個 IP）`),await this.updatePageContent()});const r=this.container.querySelector("#api-config-form");r&&r.addEventListener("submit",p=>{var D,O,T,P,z,L;p.preventDefault(),p.stopPropagation();const h=((D=this.container.querySelector("#llm-endpoint"))==null?void 0:D.value)||"",u=((O=this.container.querySelector("#llm-api-key"))==null?void 0:O.value)||"",g=((T=this.container.querySelector("#llm-deployment"))==null?void 0:T.value)||"",b=((P=this.container.querySelector("#embed-endpoint"))==null?void 0:P.value)||"",w=((z=this.container.querySelector("#embed-api-key"))==null?void 0:z.value)||"",C=((L=this.container.querySelector("#embed-deployment"))==null?void 0:L.value)||"",M={azureOpenAI:{endpoint:h,apiKey:u,deployment:g,embeddingDeployment:C},llmAPI:{endpoint:h,apiKey:u,deployment:g},embeddingAPI:{endpoint:b,apiKey:w,deployment:C}};f.saveConfig(M),alert("API 設定已儲存")});const s=this.container.querySelector("#agent-tool-config-form");s&&s.addEventListener("submit",async p=>{var b,w;p.preventDefault(),p.stopPropagation();const h=((b=this.container.querySelector("#manual-index-enabled"))==null?void 0:b.checked)||!1,u=((w=this.container.querySelector("#frontend-pages-enabled"))==null?void 0:w.checked)||!1,g=f.loadAgentToolConfig();g&&(g.manualIndex.enabled=h,g.frontendPages.enabled=u,f.saveAgentToolConfig(g),alert("Agent 設定已儲存"),await this.updatePageContent())});const a=this.container.querySelector("#sql-plugin-config-form");a&&a.addEventListener("submit",async p=>{var T,P,z,L,F,U,j,H;p.preventDefault(),p.stopPropagation();const h=((T=this.container.querySelector("#sql-plugin-enabled"))==null?void 0:T.checked)||!1,u=parseInt(((P=this.container.querySelector("#sql-plugin-priority"))==null?void 0:P.value)||"5"),g=((z=this.container.querySelector("#sql-api-endpoint"))==null?void 0:z.value)||"",b=((L=this.container.querySelector("#sql-connection-id"))==null?void 0:L.value)||"",w=((F=this.container.querySelector("#sql-search-table"))==null?void 0:F.value)||"knowledge_base",C=((U=this.container.querySelector("#sql-title-column"))==null?void 0:U.value)||"title",M=((j=this.container.querySelector("#sql-content-column"))==null?void 0:j.value)||"content",D=((H=this.container.querySelector("#sql-url-column"))==null?void 0:H.value)||"url",O={enabled:h,priority:u,apiEndpoint:g,connectionId:b,searchTable:w,titleColumn:C,contentColumn:M,urlColumn:D};localStorage.setItem("sm_sql_plugin_config",JSON.stringify(O)),alert("SQL Plugin 設定已儲存"),await this.updatePageContent()});const l=this.container.querySelector("#sql-connection-form");l&&l.addEventListener("submit",async p=>{var g,b;p.preventDefault(),p.stopPropagation();const h=((g=this.container.querySelector("#sql-conn-name"))==null?void 0:g.value)||"",u=(b=this.container.querySelector("#sql-conn-type"))==null?void 0:b.value;if(!h){alert("請輸入連接名稱");return}try{A.create({name:h,type:u,host:"localhost",port:3306,database:"mydb",username:"user",password:"password",queryTemplate:"SELECT * FROM {table} WHERE {conditions}",resultMapping:{titleField:"title",contentField:"content",urlField:"url"}}),alert("SQL 連接已新增"),await this.updatePageContent()}catch(w){console.error("Error creating SQL connection:",w),alert("新增失敗")}}),this.container.querySelectorAll(".delete-sql-connection").forEach(p=>{p.addEventListener("click",async()=>{const h=p.dataset.id;if(h&&confirm("確定要刪除這個連接嗎？"))try{A.delete(h),alert("連接已刪除"),await this.updatePageContent()}catch(u){console.error("Error deleting SQL connection:",u),alert("刪除失敗")}})})}renderAdminUI(){return`
          ...
          <div style="margin-bottom: 24px;">
            ...
            <div style="margin-bottom: 16px;">
              ...
              <input
                ...
                value="${((u=e.embeddingAPI)==null?void 0:u.deployment)||((g=e.azureOpenAI)==null?void 0:g.embeddingDeployment)||""}"
                style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; box-sizing: border-box; background: white; color: #1f2937;"
              />
            </div>
          </div>
    ...
    `,document.body.appendChild(n);const o=n.querySelector("#edit-index-form"),i=n.querySelector("#cancel-edit-btn");o.addEventListener("submit",async r=>{r.preventDefault();const s=n.querySelector("#edit-index-name").value,a=n.querySelector("#edit-index-description").value,l=n.querySelector("#edit-index-content").value;if(!s||!l){await this.showAlertDialog("請填寫名稱和內容");return}try{await x.update(e,{name:s,description:a,content:l}),await this.showAlertDialog("索引已更新"),document.body.removeChild(n),await this.updatePageContent()}catch(c){await this.showAlertDialog(`更新失敗：${c instanceof Error?c.message:"未知錯誤"}`)}}),i.addEventListener("click",()=>{document.body.removeChild(n)}),n.addEventListener("click",r=>{r.target===n&&document.body.removeChild(n)})}async showAddIndexModal(){const e=document.createElement("div");e.style.cssText=`
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
    ...
    `,document.body.appendChild(e);const t=e.querySelector("#add-index-form"),n=e.querySelector("#cancel-add-btn");t.addEventListener("submit",async o=>{o.preventDefault();const i=e.querySelector("#add-index-name").value,r=e.querySelector("#add-index-description").value,s=e.querySelector("#add-index-url").value,a=e.querySelector("#add-index-content").value;if(!i||!a){await this.showAlertDialog("請填寫名稱和內容");return}try{await x.create({name:i,description:r,content:a,url:s||void 0}),await this.showAlertDialog("索引已新增"),document.body.removeChild(e),await this.updatePageContent()}catch(l){await this.showAlertDialog(`新增失敗：${l instanceof Error?l.message:"未知錯誤"}`)}}),n.addEventListener("click",()=>{document.body.removeChild(e)}),e.addEventListener("click",o=>{o.target===e&&document.body.removeChild(e)})}async showDeleteConfirmDialog(e){const t=await x.getById(e);if(!t){await this.showAlertDialog("找不到該索引");return}if(await this.showConfirmDialog(`確定要刪除索引「${t.name}」嗎？此操作無法復原。`))try{await x.delete(e),await this.showAlertDialog("索引已刪除"),await this.updatePageContent()}catch(o){await this.showAlertDialog(`刪除失敗：${o instanceof Error?o.message:"未知錯誤"}`)}}async renderConversations(){try{const{CustomerServiceManager:e}=await Promise.resolve().then(()=>_),t=await e.getAllConversations();return`
      ...
      `}}async renderAdminUsers(){try{const{AdminUserManager:e}=await Promise.resolve().then(()=>se),t=await e.getAllAdminUsers();return`
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
      ...
      `}}async renderSystemSettings(){var i,r;let e=[],t=[];try{const{DatabaseService:s}=await Promise.resolve().then(()=>$),[a,l]=await Promise.all([s.getSettings().catch(()=>[]),s.getAdminUsers().catch(()=>[])]);e=a,t=l}catch(s){console.error("Failed to load system settings:",s)}const n=((i=e.find(s=>s.key==="default_reply"))==null?void 0:i.value)||"",o=((r=e.find(s=>s.key==="system_prompt"))==null?void 0:r.value)||"";return`
    ...
    `,document.body.appendChild(e);const t=e.querySelector("#add-admin-user-form"),n=e.querySelector("#cancel-add-admin-btn");t.addEventListener("submit",async o=>{o.preventDefault();const i=e.querySelector("#add-admin-username").value,r=e.querySelector("#add-admin-password").value,s=e.querySelector("#add-admin-email").value;try{const{DatabaseService:a}=await Promise.resolve().then(()=>$);await a.createAdminUser(i,r,s),document.body.removeChild(e),await this.showAlertDialog("管理員帳號已新增"),await this.updatePageContent()}catch(a){await this.showAlertDialog(`新增失敗：${a instanceof Error?a.message:"未知錯誤"}`)}}),n.addEventListener("click",()=>{document.body.removeChild(e)}),e.addEventListener("click",o=>{o.target===e&&document.body.removeChild(e)})}async showConversationModal(e){var t;try{const{CustomerServiceManager:n}=await Promise.resolve().then(()=>_),o=await n.getConversationById(e);if(!o){await this.showAlertDialog("找不到該對話記錄");return}const i=document.createElement("div");i.style.cssText=`
      ...
      `,document.body.appendChild(i);const r=i.querySelector("#close-conversation-modal"),s=i.querySelector("#close-conversation-modal-btn"),a=()=>{document.body.removeChild(i)};r==null||r.addEventListener("click",a),s==null||s.addEventListener("click",a),i.addEventListener("click",l=>{l.target===i&&a()})}catch(n){console.error("Error showing conversation modal:",n),await this.showAlertDialog("載入對話詳情失敗")}}}class q{static getCurrentUser(){const e=localStorage.getItem(this.USER_KEY);if(e){const t=JSON.parse(e),n=this.getOrCreateSessionId();return t.sessionId=n,t.metadata.lastSeen=Date.now(),this.saveUser(t),t}return this.createNewUser()}static createNewUser(){const e=this.generateUserId(),t=this.getOrCreateSessionId(),n={id:e,sessionId:t,metadata:{userAgent:navigator.userAgent,firstSeen:Date.now(),lastSeen:Date.now(),totalConversations:0}};return this.saveUser(n),console.log("Created new user:",n.id),n}static saveUser(e){localStorage.setItem(this.USER_KEY,JSON.stringify(e))}static getOrCreateSessionId(){let e=sessionStorage.getItem(this.SESSION_KEY);return e||(e=this.generateSessionId(),sessionStorage.setItem(this.SESSION_KEY,e)),e}static generateUserId(){return"user_"+this.generateRandomId()}static generateSessionId(){return"session_"+this.generateRandomId()}static generateRandomId(){return Date.now().toString(36)+Math.random().toString(36).substring(2)}static incrementConversationCount(){const e=this.getCurrentUser();e.metadata.totalConversations++,this.saveUser(e)}static getUserId(){return this.getCurrentUser().id}static getSessionId(){return this.getCurrentUser().sessionId}}d(q,"USER_KEY","sm_user"),d(q,"SESSION_KEY","sm_session");class Y{constructor(){d(this,"plugins",new Map)}register(e){this.plugins.has(e.id)&&console.warn(`Plugin ${e.id} already registered, replacing...`),this.plugins.set(e.id,e),console.log(`✅ Plugin registered: ${e.name} (${e.id})`)}unregister(e){const t=this.plugins.get(e);t&&(t.dispose(),this.plugins.delete(e),console.log(`❌ Plugin unregistered: ${t.name} (${e})`))}getPlugin(e){return this.plugins.get(e)}getAllPlugins(){return Array.from(this.plugins.values())}async getEnabledPlugins(){const e=this.getAllPlugins(),t=[];for(const n of e)n.enabled&&await n.isAvailable()&&t.push(n);return t.sort((n,o)=>o.priority-n.priority)}async initializeAll(){const e=this.getAllPlugins();console.log(`🔌 Initializing ${e.length} plugins...`),await Promise.all(e.map(async t=>{try{await t.initialize(),console.log(`✅ Plugin initialized: ${t.name}`)}catch(n){console.error(`❌ Failed to initialize plugin ${t.name}:`,n)}}))}async search(e,t=5){const n=await this.getEnabledPlugins();if(n.length===0)return console.warn("No enabled plugins available for search"),[];console.log(`🔍 Searching with ${n.length} plugins:`,n.map(r=>r.name));const i=(await Promise.all(n.map(async r=>{try{return(await r.search(e,t)).map(a=>({...a,metadata:{...a.metadata,pluginId:r.id,pluginName:r.name,priority:r.priority}}))}catch(s){return console.error(`Error searching with plugin ${r.name}:`,s),[]}}))).flat();return i.sort((r,s)=>{var c,p;const a=((c=r.metadata)==null?void 0:c.priority)||0,l=((p=s.metadata)==null?void 0:p.priority)||0;return a!==l?l-a:(s.score||0)-(r.score||0)}),i.slice(0,t)}disposeAll(){this.plugins.forEach(e=>e.dispose()),this.plugins.clear(),console.log("🧹 All plugins disposed")}}class G{constructor(){d(this,"id","manual-index");d(this,"name","手動索引");d(this,"description","搜尋管理員手動新增的索引內容");d(this,"priority",10);d(this,"enabled",!0)}async initialize(){const e=await x.getAll();console.log(`📚 Manual Index Plugin: ${e.length} indexes loaded`)}async search(e,t=5){try{return(await x.search(e,t)).map(({index:o,score:i,breakdown:r})=>({type:"manual-index",title:o.name,snippet:o.content.substring(0,200),content:o.content,url:`#manual-index-${o.id}`,score:i,metadata:{description:o.description,createdAt:o.createdAt,indexId:o.id,hasEmbedding:!!o.embedding,scoreBreakdown:r}}))}catch(n){return console.error("Error in ManualIndexPlugin.search:",n),[]}}async isAvailable(){return(await x.getAll()).length>0}async getConfig(){const e=await x.getAll();return{enabled:this.enabled,priority:this.priority,indexCount:e.length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class V{constructor(){d(this,"id","frontend-pages");d(this,"name","前端頁面");d(this,"description","搜尋當前網站已索引的頁面內容");d(this,"priority",8);d(this,"enabled",!0);d(this,"extractor");this.extractor=new S}async initialize(){const e=f.loadIndexedPages();console.log(`📄 Frontend Page Plugin: ${e.length} pages loaded`)}async search(e,t=5){try{const n=f.loadIndexedPages();if(n.length===0)return[];const o=this.extractor.extractKeywords(e);return n.map(r=>{const s=`${r.title} ${r.snippet}`.toLowerCase(),l=o.filter(c=>s.includes(c.toLowerCase())).length/o.length;return{page:r,score:l}}).filter(r=>r.score>0).sort((r,s)=>s.score-r.score).slice(0,t).map(({page:r,score:s})=>({type:"frontend-page",title:r.title,snippet:r.snippet,content:r.snippet,url:r.url,score:s,metadata:{keywords:r.keywords,pageId:r.id}}))}catch(n){return console.error("Error in FrontendPagePlugin.search:",n),[]}}isAvailable(){return f.loadIndexedPages().length>0}getConfig(){return{enabled:this.enabled,priority:this.priority,pageCount:f.loadIndexedPages().length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class I{static getAll(){const e=localStorage.getItem(this.STORAGE_KEY);if(!e)return[];try{return JSON.parse(e)}catch(t){return console.error("Failed to parse sitemap configs:",t),[]}}static getById(e){return this.getAll().find(n=>n.id===e)||null}static async create(e){const t={id:this.generateId(),domain:e.domain,sitemapUrl:e.sitemapUrl,enabled:!0,autoUpdate:e.autoUpdate||!1,updateInterval:e.updateInterval||60,lastUpdated:0,pages:[]},n=this.getAll();return n.push(t),this.saveAll(n),console.log("Created sitemap config:",t.id),await this.crawl(t.id),t.autoUpdate&&this.startAutoUpdate(t.id),t}static update(e,t){const n=this.getAll(),o=n.find(i=>i.id===e);return o?(t.domain!==void 0&&(o.domain=t.domain),t.sitemapUrl!==void 0&&(o.sitemapUrl=t.sitemapUrl),t.enabled!==void 0&&(o.enabled=t.enabled),t.autoUpdate!==void 0&&(o.autoUpdate=t.autoUpdate),t.updateInterval!==void 0&&(o.updateInterval=t.updateInterval),this.saveAll(n),o.autoUpdate?this.startAutoUpdate(e):this.stopAutoUpdate(e),console.log("Updated sitemap config:",e),o):null}static delete(e){const t=this.getAll(),n=t.filter(o=>o.id!==e);return n.length===t.length?!1:(this.saveAll(n),this.stopAutoUpdate(e),console.log("Deleted sitemap config:",e),!0)}static async crawl(e){const t=this.getById(e);if(!t)throw new Error("Sitemap config not found");console.log("Crawling sitemap:",t.sitemapUrl);try{const o=await(await fetch(t.sitemapUrl)).text(),r=new DOMParser().parseFromString(o,"text/xml"),s=Array.from(r.querySelectorAll("url loc")).map(u=>u.textContent||"");console.log(`Found ${s.length} URLs in sitemap`);const l=s.slice(0,50),c=[];for(const u of l)try{const g=await this.crawlPage(u);g&&c.push(g)}catch(g){console.error(`Failed to crawl ${u}:`,g)}t.pages=c,t.lastUpdated=Date.now();const p=this.getAll(),h=p.findIndex(u=>u.id===e);h>=0&&(p[h]=t,this.saveAll(p)),console.log(`Crawled ${c.length} pages successfully`)}catch(n){throw console.error("Failed to crawl sitemap:",n),n}}static async crawlPage(e){var t;try{const o=await(await fetch(e)).text(),r=new DOMParser().parseFromString(o,"text/html"),s=((t=r.querySelector("title"))==null?void 0:t.textContent)||e,a=new S,l=a.extractText(r.body),c=a.extractKeywords(l),p=a.generateFingerprint(l);return{url:e,title:s,content:l.substring(0,5e3),keywords:c,fingerprint:p,lastCrawled:Date.now()}}catch(n){return console.error(`Failed to crawl page ${e}:`,n),null}}static search(e,t,n=5){const o=this.getAll().filter(c=>c.enabled),i=t&&t.length>0?o.filter(c=>t.includes(c.domain)):o;if(i.length===0)return[];const r=new S,s=r.extractKeywords(e),a=r.generateFingerprint(e),l=[];for(const c of i)for(const p of c.pages){const h=this.calculateSimilarity(s,a,p.keywords,p.fingerprint);h>0&&l.push({page:p,domain:c.domain,score:h})}return l.sort((c,p)=>p.score-c.score).slice(0,n)}static calculateSimilarity(e,t,n,o){const i=this.calculateKeywordScore(e,n),r=this.calculateFingerprintScore(t,o);return i*.5+r*.5}static calculateKeywordScore(e,t){return e.length===0||t.length===0?0:e.filter(o=>t.includes(o)).length/Math.max(e.length,t.length)}static calculateFingerprintScore(e,t){if(e.length===0||t.length===0)return 0;let n=0,o=0;for(let i=0;i<Math.max(e.length,t.length);i++){const r=e[i]||0,s=t[i]||0;r===1&&s===1&&n++,(r===1||s===1)&&o++}return o>0?n/o:0}static startAutoUpdate(e){this.stopAutoUpdate(e);const t=this.getById(e);if(!t||!t.autoUpdate)return;const n=t.updateInterval*60*1e3,o=window.setInterval(()=>{console.log(`Auto-updating sitemap: ${e}`),this.crawl(e).catch(i=>console.error("Auto-update failed:",i))},n);this.updateTimers.set(e,o)}static stopAutoUpdate(e){const t=this.updateTimers.get(e);t&&(clearInterval(t),this.updateTimers.delete(e))}static initAutoUpdates(){const e=this.getAll().filter(t=>t.enabled&&t.autoUpdate);for(const t of e)this.startAutoUpdate(t.id);console.log(`Initialized ${e.length} auto-update timers`)}static saveAll(e){localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}static generateId(){return"sitemap_"+Date.now().toString(36)+Math.random().toString(36).substring(2)}static clearAll(){this.updateTimers.forEach(e=>clearInterval(e)),this.updateTimers.clear(),localStorage.removeItem(this.STORAGE_KEY)}}d(I,"STORAGE_KEY","sm_sitemap_configs"),d(I,"updateTimers",new Map);class X{constructor(){d(this,"id","sitemap");d(this,"name","Sitemap 索引");d(this,"description","搜尋外部網站的 Sitemap 內容");d(this,"priority",6);d(this,"enabled",!1);d(this,"extractor");this.extractor=new S}async initialize(){const e=I.getAll();console.log(`🗺️ Sitemap Plugin: ${e.length} sitemaps loaded`),e.length>0&&(this.enabled=!0)}async search(e,t=5){try{const n=I.getAll();if(n.length===0)return[];const o=[],i=this.extractor.extractKeywords(e);for(const r of n)try{const s=await I.search(r.id,i,3);o.push(...s.map(({page:a,score:l})=>({type:"sitemap",title:a.title,snippet:a.content.substring(0,200),content:a.content.substring(0,500),url:a.url,score:l,metadata:{domain:r.domain,lastUpdated:r.lastUpdated,sitemapId:r.id}})))}catch(s){console.error(`Error searching sitemap ${r.domain}:`,s)}return o.sort((r,s)=>(s.score||0)-(r.score||0)).slice(0,t)}catch(n){return console.error("Error in SitemapPlugin.search:",n),[]}}isAvailable(){return I.getAll().length>0}getConfig(){return{enabled:this.enabled,priority:this.priority,sitemapCount:I.getAll().length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class Z{constructor(e){d(this,"id","sql-database");d(this,"name","SQL 資料庫");d(this,"description","搜尋 SQL 資料庫中的內容");d(this,"priority",5);d(this,"enabled",!1);d(this,"config");d(this,"extractor");this.config={enabled:!1,priority:5,searchTable:"knowledge_base",searchColumns:["title","content"],titleColumn:"title",contentColumn:"content",urlColumn:"url",...e},this.enabled=this.config.enabled,this.priority=this.config.priority,this.extractor=new S}async initialize(){if(!this.config.connectionId){console.warn("⚠️ SQL Plugin: No connection ID configured"),this.enabled=!1;return}if(!this.config.apiEndpoint){console.warn("⚠️ SQL Plugin: No API endpoint configured"),this.enabled=!1;return}try{if(!A.getById(this.config.connectionId)){console.warn(`⚠️ SQL Plugin: Connection ${this.config.connectionId} not found`),this.enabled=!1;return}if(!await A.testConnection(this.config.connectionId,this.config.apiEndpoint)){console.warn("⚠️ SQL Plugin: Connection test failed"),this.enabled=!1;return}console.log("✅ SQL Plugin: Connection test successful")}catch(e){console.error("❌ SQL Plugin initialization error:",e),this.enabled=!1}}async search(e,t=5){if(!this.isAvailable())return[];try{const n=this.extractor.extractKeywords(e,5),o=this.buildSearchQuery(n,t),i=await A.query(this.config.connectionId,o,this.config.apiEndpoint);return this.convertToSources(i)}catch(n){return console.error("Error in SQLPlugin.search:",n),[]}}buildSearchQuery(e,t){const{searchTable:n,searchColumns:o,titleColumn:i,contentColumn:r,urlColumn:s}=this.config,a=o.map(l=>e.map(c=>`${l} LIKE '%${c}%'`).join(" OR ")).join(" OR ");return`
      SELECT 
        ${i} as title,
        ${r} as content,
        ${s} as url
      ...
      WHERE ${a}
      LIMIT ${t}
    `.trim()}convertToSources(e){return e.map((t,n)=>({type:"sql",title:t.title||`結果 ${n+1}`,snippet:t.content?t.content.substring(0,200):"",content:t.content||"",url:t.url||"#",score:1-n*.1,metadata:{source:"sql-database",connectionId:this.config.connectionId,table:this.config.searchTable}}))}isAvailable(){return this.enabled&&!!this.config.connectionId&&!!this.config.apiEndpoint&&!!this.config.searchTable}getConfig(){return{...this.config}}updateConfig(e){this.config={...this.config,...e},typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority),this.initialize().catch(t=>{console.error("Error reinitializing SQL Plugin:",t)})}dispose(){this.enabled=!1}}function ee(){const m=localStorage.getItem("sm_sql_plugin_config"),e=m?JSON.parse(m):{};return new Z(e)}function te(){const m=new Y;return m.register(new G),m.register(new V),m.register(new X),m.register(ee()),m}function ne(m){const e=localStorage.getItem("sm_plugin_configs");if(e)try{const t=JSON.parse(e);Object.keys(t).forEach(n=>{const o=m.getPlugin(n);o&&o.updateConfig(t[n])}),console.log("✅ Plugin configs loaded from localStorage")}catch(t){console.error("Error loading plugin configs:",t)}}class oe{constructor(){d(this,"config");d(this,"openAI");d(this,"indexing");d(this,"agent");d(this,"panel");d(this,"capture");d(this,"conversationState");d(this,"initialized",!1);d(this,"captureMode",!1);d(this,"adminPanel");d(this,"pluginManager");d(this,"floatingIcon");d(this,"screenshotMode",!1);d(this,"hoverHandler",null);d(this,"mouseLeaveHandler",null)}async loadRulesFromSQL(){try{const e=await fetch("http://localhost:3002/rules");if(!e.ok)return console.log("No rules found in database, using empty array"),[];const t=await e.json();return Array.isArray(t)?t:[]}catch(e){return console.error("Failed to load rules from SQL:",e),[]}}async init(e){var o,i,r,s;if(this.initialized){console.warn("ServiceModuler already initialized");return}this.config=e,q.getCurrentUser(),console.log("User ID:",q.getUserId()),this.pluginManager=te(),ne(this.pluginManager),this.pluginManager.initializeAll().then(()=>{console.log("✅ All plugins initialized")}).catch(a=>{console.error("❌ Plugin initialization error:",a)}),this.openAI=new v(e.azureOpenAI||e.llmAPI),this.indexing=new k(this.openAI,e.siteConfig),x.setOpenAIService(this.openAI),E.setConfig({});const t=e.telegram&&e.telegram.botToken&&e.telegram.chatId?e.telegram:void 0;window.SM_TELEGRAM_CONFIG=t;const n=await this.loadRulesFromSQL();this.agent=new B(this.openAI,this.pluginManager,n,t),this.capture=new J,this.panel=new Q(((o=e.ui)==null?void 0:o.width)||"33.33%",((i=e.ui)==null?void 0:i.position)||"right"),this.panel.setCallbacks({onSendMessage:(a,l)=>this.handleSendMessage(a,l),onSelectRule:a=>this.handleSelectRule(a),onClose:()=>this.handleClose(),onOpen:()=>this.handleOpen()}),this.loadConversationState(),this.agent&&this.panel.setRules(this.agent.getRules(),(r=this.agent.getCurrentRule())==null?void 0:r.id),this.adminPanel||(this.adminPanel=new W),window.location.pathname==="/lens-service"&&this.openAdminPanel(),this.bindGlobalKeyboardShortcuts(),((s=e.ui)==null?void 0:s.iconPosition)!==!1&&this.createFloatingIcon(),this.initialized=!0,e.debug&&console.log("ServiceModuler initialized",e)}bindGlobalKeyboardShortcuts(){document.addEventListener("keydown",e=>{var t,n;e.key&&e.key.toLowerCase()==="q"&&((t=this.panel)!=null&&t.isPanelOpen())?(console.log("🎯 Q key pressed, panel is open, enabling screenshot mode"),this.enableScreenshotMode()):e.key&&e.key.toLowerCase()==="q"&&console.log("🎯 Q key pressed, but panel is not open:",(n=this.panel)==null?void 0:n.isPanelOpen())}),document.addEventListener("keyup",e=>{e.key&&e.key.toLowerCase()==="q"&&this.disableScreenshotMode()}),document.addEventListener("click",e=>{var t;this.screenshotMode&&((t=this.panel)!=null&&t.isPanelOpen())&&(console.log("📸 Screenshot click detected"),e.preventDefault(),e.stopPropagation(),this.captureElementScreenshot(e.target))},!0)}open(){var e;if(!this.initialized){console.error("ServiceModuler not initialized. Call init() first.");return}(e=this.panel)==null||e.open()}close(){var e;(e=this.panel)==null||e.close()}async sendMessage(e,t){var o,i,r,s,a;if(!this.initialized||!this.agent||!this.panel||!this.openAI){console.error("ServiceModuler not initialized");return}const n={role:"user",content:e||"請分析這張圖片",timestamp:Date.now()};(o=this.conversationState)==null||o.messages.push(n),this.panel.addMessage(n),this.saveConversationState();try{let l,c,p=!1;const h=((i=this.conversationState)==null?void 0:i.sessionId)||this.generateSessionId(),u=localStorage.getItem("lens_service_user_id")||"default_user";if(t)l=await this.openAI.chatCompletionWithImage(e||"請分析這張圖片並回答問題",t,((r=this.conversationState)==null?void 0:r.messages.slice(0,-1))||[]);else{const b=await this.agent.processMessage(e,((s=this.conversationState)==null?void 0:s.messages)||[],h,u);l=b.response,c=b.sources,p=b.needsHumanReply}const g={role:"assistant",content:l,timestamp:Date.now(),sources:c};(a=this.conversationState)==null||a.messages.push(g),this.panel.addMessage(g),this.saveConversationState(),await this.saveConversationToDatabase(h,u)}catch(l){console.error("Error processing message:",l);const c={role:"assistant",content:`抱歉，發生錯誤：${l instanceof Error?l.message:"未知錯誤"}`,timestamp:Date.now()};this.panel.addMessage(c)}}async saveConversationToDatabase(e,t){if(this.conversationState)try{if(!(await fetch("/api/conversations",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:t,conversationId:e,messages:this.conversationState.messages})})).ok)throw new Error("Failed to save conversation");console.log("✅ Conversation saved to database")}catch(n){console.error("Failed to save conversation to database:",n)}}setRule(e){var t;this.agent&&(this.agent.setRule(e),this.panel&&this.panel.setRules(this.agent.getRules(),(t=this.agent.getCurrentRule())==null?void 0:t.id))}openAdminPanel(){this.adminPanel&&this.adminPanel.open().catch(console.error)}async indexSite(e,t="domain",n){if(!this.indexing){console.error("Indexing service not initialized");return}const o=e||window.location.origin;await this.indexing.indexSite(o,t,n)}enableCaptureMode(){if(!this.capture||!this.panel){console.error("Capture service not initialized");return}this.captureMode=!0,this.capture.enable((e,t,n)=>{console.log("Element captured:",{text:t,element:n}),this.open(),this.panel.setCapturedImage(e,t)}),console.log("Capture mode enabled. Press Ctrl+Click to capture elements.")}disableCaptureMode(){this.capture&&(this.capture.disable(),this.captureMode=!1)}searchCurrentPage(e){return R.searchInCurrentPage(e).map(n=>({text:n.text,context:n.context}))}getCurrentPageContent(){return R.extractCurrentPageContent()}clearConversation(){var e;this.conversationState&&(this.conversationState.messages=[],this.saveConversationState()),(e=this.panel)==null||e.clearMessages()}async openAdmin(){if(!this.initialized){console.error("ServiceModuler not initialized. Call init() first.");return}if(!this.adminPanel){console.error("AdminPanel not initialized");return}await this.adminPanel.open()}destroy(){var e,t;(e=this.panel)==null||e.destroy(),(t=this.adminPanel)==null||t.close(),this.initialized=!1}handleSendMessage(e,t){this.sendMessage(e,t)}handleSelectRule(e){this.setRule(e)}handleOpen(){console.log("✅ Panel opened")}handleClose(){this.saveConversationState(),console.log("❌ Panel closed")}loadConversationState(){let e=f.loadConversation();e||(e={sessionId:this.generateSessionId(),messages:[]}),this.conversationState=e,this.panel&&e.messages.length>0&&e.messages.forEach(t=>{this.panel.addMessage(t)})}saveConversationState(){this.conversationState&&f.saveConversation(this.conversationState)}createFloatingIcon(){var o,i;this.floatingIcon&&this.floatingIcon.remove();const e=(i=(o=this.config)==null?void 0:o.ui)==null?void 0:i.iconPosition;let t={bottom:"20px",right:"20px"};if(typeof e=="string")switch(e){case"bottom-left":t={bottom:"20px",left:"20px"};break;case"top-right":t={top:"20px",right:"20px"};break;case"top-left":t={top:"20px",left:"20px"};break;default:t={top:"20px",right:"20px"}}else e&&typeof e=="object"&&(t=e);this.floatingIcon=document.createElement("button"),this.floatingIcon.id="lens-service-floating-icon",this.floatingIcon.innerHTML=`
