...
如果不需要任何工具，返回：
{
  "tools": [],
  "reason": "這是一般對話，不需要搜尋"
}`;try{const o=await this.openAI.chatCompletion([{role:"system",content:n},{role:"user",content:e}],.3,500),i=JSON.parse(o);return console.log("Tool selection reason:",i.reason),i.tools||[]}catch(o){return console.error("Failed to determine tools:",o),t.map(i=>i.id)}}async generateResponse(e,t,n){var s,a,c;let o=((s=this.currentRule)==null?void 0:s.systemPrompt)||"你是一個專業的客服助手。";o+=`
...
`)}`}),r.push({role:"user",content:e});try{const d=await this.openAI.chatCompletion(r,((a=this.currentRule)==null?void 0:a.temperature)||.7,((c=this.currentRule)==null?void 0:c.maxTokens)||1e3),u=!d.includes("CANNOT_ANSWER");return{response:d.replace(/CANNOT_ANSWER/g,"").trim()||d,canAnswer:u}}catch(d){return console.error("Failed to generate response:",d),{response:"抱歉，系統暫時無法處理您的請求。",canAnswer:!1}}}getRecentQA(e,t){const n=[];let o="";for(let i=e.length-1;i>=0&&n.length<t;i--){const r=e[i];r.role==="assistant"&&o?(n.unshift(`Q: ${o}
...
`).trim()}searchRelevantSections(e,t=5){const n=this.extract(),o=e.toLowerCase().split(/\s+/);return n.sections.map(r=>{let s=r.relevance;const a=r.heading.toLowerCase();o.forEach(d=>{a.includes(d)&&(s+=5)});const c=r.content.toLowerCase();return o.forEach(d=>{const u=(c.match(new RegExp(d,"g"))||[]).length;s+=u*2}),{heading:r.heading,content:r.content,score:s}}).sort((r,s)=>s.score-r.score).slice(0,t)}extractText(e){const t=e.cloneNode(!0);return t.querySelectorAll("script, style, noscript").forEach(n=>n.remove()),t.textContent||""}extractKeywords(e,t=20){const n=e.toLowerCase().replace(/[^\w\s\u4e00-\u9fa5]/g," ").split(/\s+/).filter(i=>i.length>1),o=new Map;for(const i of n)o.set(i,(o.get(i)||0)+1);return Array.from(o.entries()).sort((i,r)=>r[1]-i[1]).slice(0,t).map(([i])=>i)}generateFingerprint(e,t=64){const n=this.extractKeywords(e,50),o=new Array(t).fill(0);for(const i of n){const r=this.simpleHash(i,t);for(let s=0;s<t;s++)r[s]===1?o[s]++:o[s]--}return o.map(i=>i>0?1:0)}simpleHash(e,t){let n=0;for(let i=0;i<e.length;i++)n=(n<<5)-n+e.charCodeAt(i),n=n&n;const o=new Array(t).fill(0);for(let i=0;i<t;i++)o[i]=n>>i&1;return o}}class I{static getAll(){const e=localStorage.getItem(this.STORAGE_KEY);if(!e)return[];try{return JSON.parse(e)}catch(t){return console.error("Failed to parse manual indexes:",t),[]}}static getById(e){return this.getAll().find(n=>n.id===e)||null}static create(e){const t=new E,n=t.extractKeywords(e.content),o=t.generateFingerprint(e.content),i={id:this.generateId(),name:e.name,description:e.description,content:e.content,keywords:n,fingerprint:o,metadata:e.metadata||{},createdAt:Date.now(),updatedAt:Date.now()},r=this.getAll();return r.push(i),this.saveAll(r),console.log("Created manual index:",i.id),i}static update(e,t){const n=this.getAll(),o=n.find(i=>i.id===e);if(!o)return null;if(t.name!==void 0&&(o.name=t.name),t.description!==void 0&&(o.description=t.description),t.metadata!==void 0&&(o.metadata=t.metadata),t.content!==void 0){o.content=t.content;const i=new E;o.keywords=i.extractKeywords(t.content),o.fingerprint=i.generateFingerprint(t.content)}return o.updatedAt=Date.now(),this.saveAll(n),console.log("Updated manual index:",e),o}static delete(e){const t=this.getAll(),n=t.filter(o=>o.id!==e);return n.length===t.length?!1:(this.saveAll(n),console.log("Deleted manual index:",e),!0)}static search(e,t=5){const n=this.getAll();if(n.length===0)return[];const o=new E,i=o.extractKeywords(e),r=o.generateFingerprint(e);return n.map(a=>{const c=this.calculateSimilarity(i,r,a.keywords,a.fingerprint);return{index:a,score:c}}).filter(a=>a.score>0).sort((a,c)=>c.score-a.score).slice(0,t)}static calculateSimilarity(e,t,n,o){const i=this.calculateKeywordScore(e,n),r=this.calculateFingerprintScore(t,o);return i*.5+r*.5}static calculateKeywordScore(e,t){return e.length===0||t.length===0?0:e.filter(o=>t.includes(o)).length/Math.max(e.length,t.length)}static calculateFingerprintScore(e,t){if(e.length===0||t.length===0)return 0;let n=0,o=0;for(let i=0;i<Math.max(e.length,t.length);i++){const r=e[i]||0,s=t[i]||0;r===1&&s===1&&n++,(r===1||s===1)&&o++}return o>0?n/o:0}static saveAll(e){localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}static generateId(){return"idx_"+Date.now().toString(36)+Math.random().toString(36).substring(2)}static clearAll(){localStorage.removeItem(this.STORAGE_KEY)}static exportToJSON(){const e=this.getAll();return JSON.stringify(e,null,2)}static importFromJSON(e){try{const t=JSON.parse(e);if(!Array.isArray(t))throw new Error("Invalid format: expected array");const o=[...this.getAll(),...t];return this.saveAll(o),console.log(`Imported ${t.length} manual indexes`),t.length}catch(t){throw console.error("Failed to import indexes:",t),t}}}l(I,"STORAGE_KEY","sm_manual_indexes");class T{static getAll(){const e=localStorage.getItem(this.STORAGE_KEY);if(!e)return[];try{return JSON.parse(e)}catch(t){return console.error("Failed to parse SQL connections:",t),[]}}static getById(e){return this.getAll().find(n=>n.id===e)||null}static create(e){const t={id:this.generateId(),name:e.name,type:e.type,enabled:!0,createdAt:new Date().toISOString(),config:{host:e.host,port:e.port,database:e.database,username:e.username,password:e.password},queryTemplate:e.queryTemplate,resultMapping:e.resultMapping},n=this.getAll();return n.push(t),this.saveAll(n),console.log("Created SQL connection:",t.id),t}static update(e,t){const n=this.getAll(),o=n.find(i=>i.id===e);return o?(t.name!==void 0&&(o.name=t.name),t.type!==void 0&&(o.type=t.type),t.enabled!==void 0&&(o.enabled=t.enabled),t.config!==void 0&&(o.config={...o.config,...t.config}),t.queryTemplate!==void 0&&(o.queryTemplate=t.queryTemplate),t.resultMapping!==void 0&&(o.resultMapping={...o.resultMapping,...t.resultMapping}),this.saveAll(n),console.log("Updated SQL connection:",e),o):null}static delete(e){const t=this.getAll(),n=t.filter(o=>o.id!==e);return n.length===t.length?!1:(this.saveAll(n),console.log("Deleted SQL connection:",e),!0)}static async testConnection(e,t){const n=this.getById(e);if(!n)throw new Error("Connection not found");try{return(await(await fetch(`${t}/sql/test`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:n.type,config:n.config})})).json()).success===!0}catch(o){return console.error("Failed to test connection:",o),!1}}static async query(e,t,n){const o=this.getById(e);if(!o||!o.enabled)return[];try{const i=o.queryTemplate.replace(/\{\{query\}\}/g,t),s=await(await fetch(`${n}/sql/query`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:o.type,config:o.config,sql:i})})).json();if(!s.success||!s.rows)throw new Error(s.error||"Query failed");return s.rows.map(a=>({title:a[o.resultMapping.titleField]||"",content:a[o.resultMapping.contentField]||"",url:o.resultMapping.urlField?a[o.resultMapping.urlField]:void 0}))}catch(i){return console.error("Failed to execute query:",i),[]}}static async search(e,t,n,o=5){const i=this.getAll().filter(a=>a.enabled),r=n&&n.length>0?i.filter(a=>n.includes(a.id)):i;if(r.length===0)return[];const s=[];for(const a of r)try{const c=await this.query(a.id,e,t);for(const d of c)s.push({...d,connectionName:a.name})}catch(c){console.error(`Failed to search connection ${a.name}:`,c)}return s.slice(0,o)}static getStats(){const e=this.getAll(),t={total:e.length,enabled:e.filter(n=>n.enabled).length,byType:{}};for(const n of e)t.byType[n.type]=(t.byType[n.type]||0)+1;return t}static saveAll(e){localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}static generateId(){return"sql_"+Date.now().toString(36)+Math.random().toString(36).substring(2)}static clearAll(){localStorage.removeItem(this.STORAGE_KEY)}static exportConfig(){const e=this.getAll().map(t=>({...t,config:{...t.config,password:"***"}}));return JSON.stringify(e,null,2)}}l(T,"STORAGE_KEY","sm_sql_connections");class j{constructor(){l(this,"container",null);l(this,"isOpen",!1);l(this,"isAuthenticated",!1);l(this,"currentPage","dashboard");this.init()}init(){this.handleRouteChange(),window.addEventListener("popstate",()=>this.handleRouteChange()),this.interceptHistory()}interceptHistory(){const e=history.pushState,t=history.replaceState;history.pushState=(...n)=>{e.apply(history,n),this.handleRouteChange()},history.replaceState=(...n)=>{t.apply(history,n),this.handleRouteChange()}}handleRouteChange(){const e=window.location.pathname;e==="/lens-service"||e.startsWith("/lens-service/")?this.open():this.isOpen&&this.close()}open(){if(!this.isOpen){if(!this.checkIPWhitelist()){alert("您的 IP 不在白名單中，無法訪問管理後台"),window.location.href="/";return}this.isOpen=!0,this.container=document.createElement("div"),this.container.id="lens-service-admin",this.container.style.cssText=`
    ...
    `}bindEvents(){if(!this.container)return;const e=this.container.querySelector("#admin-login-form");if(e){e.addEventListener("submit",async h=>{h.preventDefault(),h.stopPropagation();const g=this.container.querySelector("#admin-username"),b=this.container.querySelector("#admin-password"),v=(g==null?void 0:g.value)||"",y=(b==null?void 0:b.value)||"";console.log("Login attempt with username:",v);try{const w=await fetch("/api/admin/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:v,password:y})});if(w.ok){const A=await w.json();console.log("Login successful:",A),this.isAuthenticated=!0,this.container.innerHTML=this.renderAdminUI(),this.bindEvents()}else{const A=await w.json();alert(A.error||"登入失敗"),b.value="",b.focus()}}catch(w){console.error("Login error:",w),alert("登入失敗，請稍後再試")}});const p=this.container.querySelector("#admin-username");p&&setTimeout(()=>{p.focus()},100)}this.container.querySelectorAll(".nav-item").forEach(p=>{p.addEventListener("click",()=>{const h=p.dataset.page;if(h){this.currentPage=h;const g=this.container.querySelector("#admin-content");g&&(g.innerHTML=this.renderPageContent()),this.container.innerHTML=this.renderAdminUI(),this.bindEvents()}})});const n=this.container.querySelector("#admin-logout");n&&n.addEventListener("click",()=>{this.isAuthenticated=!1,this.container.innerHTML=this.renderLoginUI(),this.bindEvents()});const o=this.container.querySelector("#change-password-form");o&&o.addEventListener("submit",p=>{p.preventDefault(),p.stopPropagation();const h=this.container.querySelector("#new-password"),g=(h==null?void 0:h.value)||"";if(g.length<4){alert("密碼長度至少 4 個字元");return}m.saveAdminPassword(g),alert("密碼已更新");const b=this.container.querySelector("#admin-content");b&&(b.innerHTML=this.renderPageContent(),this.bindEvents())});const i=this.container.querySelector("#ip-whitelist-form");i&&i.addEventListener("submit",p=>{p.preventDefault(),p.stopPropagation();const h=this.container.querySelector("#ip-list"),b=((h==null?void 0:h.value)||"").split(`
`).map(y=>y.trim()).filter(y=>y.length>0);this.saveIPWhitelist(b),alert(`已更新 IP 白名單（${b.length} 個 IP）`);const v=this.container.querySelector("#admin-content");v&&(v.innerHTML=this.renderPageContent(),this.bindEvents())});const r=this.container.querySelector("#add-manual-index-form");r&&r.addEventListener("submit",p=>{p.preventDefault(),p.stopPropagation();const h=this.container.querySelector("#index-name"),g=this.container.querySelector("#index-description"),b=this.container.querySelector("#index-content"),v=(h==null?void 0:h.value)||"",y=(g==null?void 0:g.value)||"",w=(b==null?void 0:b.value)||"";if(!v||!w){alert("請填寫名稱和內容");return}try{I.create({name:v,description:y,content:w}),alert("索引已新增");const A=this.container.querySelector("#admin-content");A&&(A.innerHTML=this.renderPageContent(),this.bindEvents())}catch(A){alert(`新增失敗：${A instanceof Error?A.message:"未知錯誤"}`)}}),this.container.querySelectorAll(".delete-index-btn").forEach(p=>{p.addEventListener("click",()=>{const h=p.dataset.id;if(h&&confirm("確定要刪除這個索引嗎？"))try{I.delete(h),alert("索引已刪除");const g=this.container.querySelector("#admin-content");g&&(g.innerHTML=this.renderPageContent(),this.bindEvents())}catch(g){alert(`刪除失敗：${g instanceof Error?g.message:"未知錯誤"}`)}})});const a=this.container.querySelector("#api-config-form");a&&a.addEventListener("submit",p=>{var R,N,$,q,L,M;p.preventDefault(),p.stopPropagation();const h=((R=this.container.querySelector("#llm-endpoint"))==null?void 0:R.value)||"",g=((N=this.container.querySelector("#llm-api-key"))==null?void 0:N.value)||"",b=(($=this.container.querySelector("#llm-deployment"))==null?void 0:$.value)||"",v=((q=this.container.querySelector("#embed-endpoint"))==null?void 0:q.value)||"",y=((L=this.container.querySelector("#embed-api-key"))==null?void 0:L.value)||"",w=((M=this.container.querySelector("#embed-deployment"))==null?void 0:M.value)||"",A={azureOpenAI:{endpoint:h,apiKey:g,deployment:b,embeddingDeployment:w},llmAPI:{endpoint:h,apiKey:g,deployment:b},embeddingAPI:{endpoint:v,apiKey:y,deployment:w}};m.saveConfig(A),alert("API 設定已儲存")});const c=this.container.querySelector("#agent-tool-config-form");c&&c.addEventListener("submit",p=>{var v,y;p.preventDefault(),p.stopPropagation();const h=((v=this.container.querySelector("#manual-index-enabled"))==null?void 0:v.checked)||!1,g=((y=this.container.querySelector("#frontend-pages-enabled"))==null?void 0:y.checked)||!1,b=m.loadAgentToolConfig();if(b){b.manualIndex.enabled=h,b.frontendPages.enabled=g,m.saveAgentToolConfig(b),alert("Agent 設定已儲存");const w=this.container.querySelector("#admin-content");w&&(w.innerHTML=this.renderPageContent(),this.bindEvents())}});const d=this.container.querySelector("#sql-plugin-config-form");d&&d.addEventListener("submit",p=>{var q,L,M,F,U,K,Q,B;p.preventDefault(),p.stopPropagation();const h=((q=this.container.querySelector("#sql-plugin-enabled"))==null?void 0:q.checked)||!1,g=parseInt(((L=this.container.querySelector("#sql-plugin-priority"))==null?void 0:L.value)||"5"),b=((M=this.container.querySelector("#sql-api-endpoint"))==null?void 0:M.value)||"",v=((F=this.container.querySelector("#sql-connection-id"))==null?void 0:F.value)||"",y=((U=this.container.querySelector("#sql-search-table"))==null?void 0:U.value)||"knowledge_base",w=((K=this.container.querySelector("#sql-title-column"))==null?void 0:K.value)||"title",A=((Q=this.container.querySelector("#sql-content-column"))==null?void 0:Q.value)||"content",R=((B=this.container.querySelector("#sql-url-column"))==null?void 0:B.value)||"url",N={enabled:h,priority:g,apiEndpoint:b,connectionId:v,searchTable:y,titleColumn:w,contentColumn:A,urlColumn:R};localStorage.setItem("sm_sql_plugin_config",JSON.stringify(N)),alert("SQL Plugin 設定已儲存");const $=this.container.querySelector("#admin-content");$&&($.innerHTML=this.renderPageContent(),this.bindEvents())});const u=this.container.querySelector("#sql-connection-form");u&&u.addEventListener("submit",p=>{var b,v;p.preventDefault(),p.stopPropagation();const h=((b=this.container.querySelector("#sql-conn-name"))==null?void 0:b.value)||"",g=(v=this.container.querySelector("#sql-conn-type"))==null?void 0:v.value;if(!h){alert("請輸入連接名稱");return}try{T.create({name:h,type:g,host:"localhost",port:3306,database:"mydb",username:"user",password:"password",queryTemplate:"SELECT * FROM {table} WHERE {conditions}",resultMapping:{titleField:"title",contentField:"content",urlField:"url"}}),alert("SQL 連接已新增");const y=this.container.querySelector("#admin-content");y&&(y.innerHTML=this.renderPageContent(),this.bindEvents())}catch(y){console.error("Error creating SQL connection:",y),alert("新增失敗")}}),this.container.querySelectorAll(".delete-sql-connection").forEach(p=>{p.addEventListener("click",()=>{const h=p.dataset.id;if(h&&confirm("確定要刪除這個連接嗎？"))try{T.delete(h),alert("連接已刪除");const g=this.container.querySelector("#admin-content");g&&(g.innerHTML=this.renderPageContent(),this.bindEvents())}catch(g){console.error("Error deleting SQL connection:",g),alert("刪除失敗")}})})}renderAdminUI(){return`
    ...
    `}renderSQL(){const e=T.getAll(),t=this.loadSQLPluginConfig();return`
    ...
    `}renderAgentAndAPI(){var n,o,i,r,s,a,c,d,u,S,p,h;const e=m.loadConfig()||{},t=m.loadAgentToolConfig();return`
          ...
          <div style="margin-bottom: 24px;">
            ...
            <div style="margin-bottom: 16px;">
              ...
              <input
                ...
                name="embedEndpoint"
                placeholder="https://your-resource.openai.azure.com/"
                value="${((c=e.embeddingAPI)==null?void 0:c.endpoint)||((d=e.azureOpenAI)==null?void 0:d.endpoint)||""}"
                style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; box-sizing: border-box; background: white; color: #1f2937;"
              />
            </div>
            ...
            <div style="margin-bottom: 16px;">
              ...
              <input
                ...
                name="embedApiKey"
                placeholder="your-api-key"
                value="${((u=e.embeddingAPI)==null?void 0:u.apiKey)||((S=e.azureOpenAI)==null?void 0:S.apiKey)||""}"
                style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; box-sizing: border-box; background: white; color: #1f2937;"
              />
            </div>
...
`)}</textarea>
    ...
    `}}class V{constructor(){l(this,"plugins",new Map)}register(e){this.plugins.has(e.id)&&console.warn(`Plugin ${e.id} already registered, replacing...`),this.plugins.set(e.id,e),console.log(`✅ Plugin registered: ${e.name} (${e.id})`)}unregister(e){const t=this.plugins.get(e);t&&(t.dispose(),this.plugins.delete(e),console.log(`❌ Plugin unregistered: ${t.name} (${e})`))}getPlugin(e){return this.plugins.get(e)}getAllPlugins(){return Array.from(this.plugins.values())}getEnabledPlugins(){return this.getAllPlugins().filter(e=>e.enabled&&e.isAvailable()).sort((e,t)=>t.priority-e.priority)}async initializeAll(){const e=this.getAllPlugins();console.log(`🔌 Initializing ${e.length} plugins...`),await Promise.all(e.map(async t=>{try{await t.initialize(),console.log(`✅ Plugin initialized: ${t.name}`)}catch(n){console.error(`❌ Failed to initialize plugin ${t.name}:`,n)}}))}async search(e,t=5){const n=this.getEnabledPlugins();if(n.length===0)return console.warn("No enabled plugins available for search"),[];console.log(`🔍 Searching with ${n.length} plugins:`,n.map(r=>r.name));const i=(await Promise.all(n.map(async r=>{try{return(await r.search(e,t)).map(a=>({...a,metadata:{...a.metadata,pluginId:r.id,pluginName:r.name,priority:r.priority}}))}catch(s){return console.error(`Error searching with plugin ${r.name}:`,s),[]}}))).flat();return i.sort((r,s)=>{var d,u;const a=((d=r.metadata)==null?void 0:d.priority)||0,c=((u=s.metadata)==null?void 0:u.priority)||0;return a!==c?c-a:(s.score||0)-(r.score||0)}),i.slice(0,t)}disposeAll(){this.plugins.forEach(e=>e.dispose()),this.plugins.clear(),console.log("🧹 All plugins disposed")}}class W{constructor(){l(this,"id","manual-index");l(this,"name","手動索引");l(this,"description","搜尋管理員手動新增的索引內容");l(this,"priority",10);l(this,"enabled",!0)}async initialize(){const e=I.getAll();console.log(`📚 Manual Index Plugin: ${e.length} indexes loaded`)}async search(e,t=5){try{return I.search(e,t).map(({index:o,score:i})=>({type:"manual-index",title:o.name,snippet:o.content.substring(0,200),content:o.content,url:`#manual-index-${o.id}`,score:i,metadata:{description:o.description,createdAt:o.createdAt,indexId:o.id}}))}catch(n){return console.error("Error in ManualIndexPlugin.search:",n),[]}}isAvailable(){return I.getAll().length>0}getConfig(){return{enabled:this.enabled,priority:this.priority,indexCount:I.getAll().length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class G{constructor(){l(this,"id","frontend-pages");l(this,"name","前端頁面");l(this,"description","搜尋當前網站已索引的頁面內容");l(this,"priority",8);l(this,"enabled",!0);l(this,"extractor");this.extractor=new E}async initialize(){const e=m.loadIndexedPages();console.log(`📄 Frontend Page Plugin: ${e.length} pages loaded`)}async search(e,t=5){try{const n=m.loadIndexedPages();if(n.length===0)return[];const o=this.extractor.extractKeywords(e);return n.map(r=>{const s=`${r.title} ${r.snippet}`.toLowerCase(),c=o.filter(d=>s.includes(d.toLowerCase())).length/o.length;return{page:r,score:c}}).filter(r=>r.score>0).sort((r,s)=>s.score-r.score).slice(0,t).map(({page:r,score:s})=>({type:"frontend-page",title:r.title,snippet:r.snippet,content:r.snippet,url:r.url,score:s,metadata:{keywords:r.keywords,pageId:r.id}}))}catch(n){return console.error("Error in FrontendPagePlugin.search:",n),[]}}isAvailable(){return m.loadIndexedPages().length>0}getConfig(){return{enabled:this.enabled,priority:this.priority,pageCount:m.loadIndexedPages().length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class k{static getAll(){const e=localStorage.getItem(this.STORAGE_KEY);if(!e)return[];try{return JSON.parse(e)}catch(t){return console.error("Failed to parse sitemap configs:",t),[]}}static getById(e){return this.getAll().find(n=>n.id===e)||null}static async create(e){const t={id:this.generateId(),domain:e.domain,sitemapUrl:e.sitemapUrl,enabled:!0,autoUpdate:e.autoUpdate||!1,updateInterval:e.updateInterval||60,lastUpdated:0,pages:[]},n=this.getAll();return n.push(t),this.saveAll(n),console.log("Created sitemap config:",t.id),await this.crawl(t.id),t.autoUpdate&&this.startAutoUpdate(t.id),t}static update(e,t){const n=this.getAll(),o=n.find(i=>i.id===e);return o?(t.domain!==void 0&&(o.domain=t.domain),t.sitemapUrl!==void 0&&(o.sitemapUrl=t.sitemapUrl),t.enabled!==void 0&&(o.enabled=t.enabled),t.autoUpdate!==void 0&&(o.autoUpdate=t.autoUpdate),t.updateInterval!==void 0&&(o.updateInterval=t.updateInterval),this.saveAll(n),o.autoUpdate?this.startAutoUpdate(e):this.stopAutoUpdate(e),console.log("Updated sitemap config:",e),o):null}static delete(e){const t=this.getAll(),n=t.filter(o=>o.id!==e);return n.length===t.length?!1:(this.saveAll(n),this.stopAutoUpdate(e),console.log("Deleted sitemap config:",e),!0)}static async crawl(e){const t=this.getById(e);if(!t)throw new Error("Sitemap config not found");console.log("Crawling sitemap:",t.sitemapUrl);try{const o=await(await fetch(t.sitemapUrl)).text(),r=new DOMParser().parseFromString(o,"text/xml"),s=Array.from(r.querySelectorAll("url loc")).map(p=>p.textContent||"");console.log(`Found ${s.length} URLs in sitemap`);const c=s.slice(0,50),d=[];for(const p of c)try{const h=await this.crawlPage(p);h&&d.push(h)}catch(h){console.error(`Failed to crawl ${p}:`,h)}t.pages=d,t.lastUpdated=Date.now();const u=this.getAll(),S=u.findIndex(p=>p.id===e);S>=0&&(u[S]=t,this.saveAll(u)),console.log(`Crawled ${d.length} pages successfully`)}catch(n){throw console.error("Failed to crawl sitemap:",n),n}}static async crawlPage(e){var t;try{const o=await(await fetch(e)).text(),r=new DOMParser().parseFromString(o,"text/html"),s=((t=r.querySelector("title"))==null?void 0:t.textContent)||e,a=new E,c=a.extractText(r.body),d=a.extractKeywords(c),u=a.generateFingerprint(c);return{url:e,title:s,content:c.substring(0,5e3),keywords:d,fingerprint:u,lastCrawled:Date.now()}}catch(n){return console.error(`Failed to crawl page ${e}:`,n),null}}static search(e,t,n=5){const o=this.getAll().filter(d=>d.enabled),i=t&&t.length>0?o.filter(d=>t.includes(d.domain)):o;if(i.length===0)return[];const r=new E,s=r.extractKeywords(e),a=r.generateFingerprint(e),c=[];for(const d of i)for(const u of d.pages){const S=this.calculateSimilarity(s,a,u.keywords,u.fingerprint);S>0&&c.push({page:u,domain:d.domain,score:S})}return c.sort((d,u)=>u.score-d.score).slice(0,n)}static calculateSimilarity(e,t,n,o){const i=this.calculateKeywordScore(e,n),r=this.calculateFingerprintScore(t,o);return i*.5+r*.5}static calculateKeywordScore(e,t){return e.length===0||t.length===0?0:e.filter(o=>t.includes(o)).length/Math.max(e.length,t.length)}static calculateFingerprintScore(e,t){if(e.length===0||t.length===0)return 0;let n=0,o=0;for(let i=0;i<Math.max(e.length,t.length);i++){const r=e[i]||0,s=t[i]||0;r===1&&s===1&&n++,(r===1||s===1)&&o++}return o>0?n/o:0}static startAutoUpdate(e){this.stopAutoUpdate(e);const t=this.getById(e);if(!t||!t.autoUpdate)return;const n=t.updateInterval*60*1e3,o=window.setInterval(()=>{console.log(`Auto-updating sitemap: ${e}`),this.crawl(e).catch(i=>console.error("Auto-update failed:",i))},n);this.updateTimers.set(e,o)}static stopAutoUpdate(e){const t=this.updateTimers.get(e);t&&(clearInterval(t),this.updateTimers.delete(e))}static initAutoUpdates(){const e=this.getAll().filter(t=>t.enabled&&t.autoUpdate);for(const t of e)this.startAutoUpdate(t.id);console.log(`Initialized ${e.length} auto-update timers`)}static saveAll(e){localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}static generateId(){return"sitemap_"+Date.now().toString(36)+Math.random().toString(36).substring(2)}static clearAll(){this.updateTimers.forEach(e=>clearInterval(e)),this.updateTimers.clear(),localStorage.removeItem(this.STORAGE_KEY)}}l(k,"STORAGE_KEY","sm_sitemap_configs"),l(k,"updateTimers",new Map);class X{constructor(){l(this,"id","sitemap");l(this,"name","Sitemap 索引");l(this,"description","搜尋外部網站的 Sitemap 內容");l(this,"priority",6);l(this,"enabled",!1);l(this,"extractor");this.extractor=new E}async initialize(){const e=k.getAll();console.log(`🗺️ Sitemap Plugin: ${e.length} sitemaps loaded`),e.length>0&&(this.enabled=!0)}async search(e,t=5){try{const n=k.getAll();if(n.length===0)return[];const o=[],i=this.extractor.extractKeywords(e);for(const r of n)try{const s=await k.search(r.id,i,3);o.push(...s.map(({page:a,score:c})=>({type:"sitemap",title:a.title,snippet:a.content.substring(0,200),content:a.content.substring(0,500),url:a.url,score:c,metadata:{domain:r.domain,lastUpdated:r.lastUpdated,sitemapId:r.id}})))}catch(s){console.error(`Error searching sitemap ${r.domain}:`,s)}return o.sort((r,s)=>(s.score||0)-(r.score||0)).slice(0,t)}catch(n){return console.error("Error in SitemapPlugin.search:",n),[]}}isAvailable(){return k.getAll().length>0}getConfig(){return{enabled:this.enabled,priority:this.priority,sitemapCount:k.getAll().length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class Z{constructor(e){l(this,"id","sql-database");l(this,"name","SQL 資料庫");l(this,"description","搜尋 SQL 資料庫中的內容");l(this,"priority",5);l(this,"enabled",!1);l(this,"config");l(this,"extractor");this.config={enabled:!1,priority:5,searchTable:"knowledge_base",searchColumns:["title","content"],titleColumn:"title",contentColumn:"content",urlColumn:"url",...e},this.enabled=this.config.enabled,this.priority=this.config.priority,this.extractor=new E}async initialize(){if(!this.config.connectionId){console.warn("⚠️ SQL Plugin: No connection ID configured"),this.enabled=!1;return}if(!this.config.apiEndpoint){console.warn("⚠️ SQL Plugin: No API endpoint configured"),this.enabled=!1;return}try{if(!T.getById(this.config.connectionId)){console.warn(`⚠️ SQL Plugin: Connection ${this.config.connectionId} not found`),this.enabled=!1;return}if(!await T.testConnection(this.config.connectionId,this.config.apiEndpoint)){console.warn("⚠️ SQL Plugin: Connection test failed"),this.enabled=!1;return}console.log("✅ SQL Plugin: Connection test successful")}catch(e){console.error("❌ SQL Plugin initialization error:",e),this.enabled=!1}}async search(e,t=5){if(!this.isAvailable())return[];try{const n=this.extractor.extractKeywords(e,5),o=this.buildSearchQuery(n,t),i=await T.query(this.config.connectionId,o,this.config.apiEndpoint);return this.convertToSources(i)}catch(n){return console.error("Error in SQLPlugin.search:",n),[]}}buildSearchQuery(e,t){const{searchTable:n,searchColumns:o,titleColumn:i,contentColumn:r,urlColumn:s}=this.config,a=o.map(c=>e.map(d=>`${c} LIKE '%${d}%'`).join(" OR ")).join(" OR ");return`
      SELECT 
        ${i} as title,
        ${r} as content,
        ${s} as url
