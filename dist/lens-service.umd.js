...
如果不需要任何工具，返回：
{
  "tools": [],
  "reason": "這是一般對話，不需要搜尋"
}`;try{const i=await this.openAI.chatCompletion([{role:"system",content:o},{role:"user",content:e}],.3,500),s=JSON.parse(i);return console.log("Tool selection reason:",s.reason),s.tools||[]}catch(i){return console.error("Failed to determine tools:",i),n.map(s=>s.id)}}async generateResponse(e,t,n){var l,p,c;const{systemPrompt:o,defaultReply:i}=await this.getSystemSettings();let s=((l=this.currentRule)==null?void 0:l.systemPrompt)||o;s+=`
...
`,n.url&&(t+=`連結：${n.url}
`),t+=`
`}),t}getSourceTypeName(e){return{"manual-index":"手動索引","frontend-page":"前端頁面",sitemap:"Sitemap",sql:"SQL 資料庫"}[e]||e}getRules(){return this.rules}getCurrentRule(){return this.currentRule}}const b={container:`
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
...
`).trim()}searchRelevantSections(e,t=5){const n=this.extract(),o=e.toLowerCase().split(/\s+/);return n.sections.map(s=>{let r=s.relevance;const a=s.heading.toLowerCase();o.forEach(p=>{a.includes(p)&&(r+=5)});const l=s.content.toLowerCase();return o.forEach(p=>{const c=(l.match(new RegExp(p,"g"))||[]).length;r+=c*2}),{heading:s.heading,content:s.content,score:r}}).sort((s,r)=>r.score-s.score).slice(0,t)}extractText(e){const t=e.cloneNode(!0);return t.querySelectorAll("script, style, noscript").forEach(n=>n.remove()),t.textContent||""}extractKeywords(e,t=20){const n=e.toLowerCase().replace(/[^\w\s\u4e00-\u9fa5]/g," ").split(/\s+/).filter(i=>i.length>1),o=new Map;for(const i of n)o.set(i,(o.get(i)||0)+1);return Array.from(o.entries()).sort((i,s)=>s[1]-i[1]).slice(0,t).map(([i])=>i)}generateFingerprint(e,t=64){const n=this.extractKeywords(e,50),o=new Array(t).fill(0);for(const i of n){const s=this.simpleHash(i,t);for(let r=0;r<t;r++)s[r]===1?o[r]++:o[r]--}return o.map(i=>i>0?1:0)}simpleHash(e,t){let n=0;for(let i=0;i<e.length;i++)n=(n<<5)-n+e.charCodeAt(i),n=n&n;const o=new Array(t).fill(0);for(let i=0;i<t;i++)o[i]=n>>i&1;return o}}class C{static setConfig(e){this.dbConfig={...this.dbConfig,...e},console.log("Database config set:",this.dbConfig)}static async query(e,t=[]){try{console.log("Executing SQL:",e,"with params:",t);const n=e.toLowerCase().trim();if(n.includes("select * from settings"))return this.mockData.settings;if(n.includes("select * from admin_users")||n.includes("select id, username, email"))return this.mockData.admin_users.map(o=>({id:o.id,username:o.username,email:o.email,created_at:o.created_at,updated_at:o.updated_at}));if(n.includes("select id, username, email from admin_users where username")){const o=t[0],i=t[1],s=this.mockData.admin_users.find(r=>r.username===o&&r.password===i);return s?[{id:s.id,username:s.username,email:s.email}]:[]}return n.includes("select * from manual_indexes")||n.includes("from manual_indexes")?this.mockData.manual_indexes:n.includes("select * from conversations")?this.mockData.conversations:[]}catch(n){throw console.error("Database query error:",n),n}}static async getSettings(){return this.query("SELECT * FROM settings ORDER BY id")}static async updateSetting(e,t){const n=this.mockData.settings.findIndex(o=>o.key===e);if(n>=0)return this.mockData.settings[n].value=t,this.mockData.settings[n].updated_at=new Date,console.log(`✅ Updated setting ${e} = ${t}`),this.mockData.settings[n];{const o={id:this.mockData.settings.length+1,key:e,value:t,created_at:new Date,updated_at:new Date};return this.mockData.settings.push(o),console.log(`✅ Created new setting ${e} = ${t}`),o}}static async saveConversation(e,t,n){const o=this.mockData.conversations.findIndex(s=>s.conversation_id===t),i={id:o>=0?this.mockData.conversations[o].id:this.mockData.conversations.length+1,user_id:e,conversation_id:t,messages:JSON.stringify(n),created_at:o>=0?this.mockData.conversations[o].created_at:new Date,updated_at:new Date};return o>=0?this.mockData.conversations[o]=i:this.mockData.conversations.push(i),console.log("✅ Conversation saved to mock database:",t),i}static async getConversations(){return this.query("SELECT * FROM conversations ORDER BY updated_at DESC")}static async deleteConversation(e){const t=this.mockData.conversations.findIndex(n=>n.conversation_id===e);t>=0&&this.mockData.conversations.splice(t,1)}static async getAdminUsers(){return this.query("SELECT id, username, email, created_at, updated_at FROM admin_users ORDER BY id")}static async createAdminUser(e,t,n){return(await this.query(`
    ...
    `}showEditDialog(e,t,n=!1){return new Promise(o=>{const i=document.createElement("div");i.style.cssText=`
      ...
      `,document.body.appendChild(i);const r=i.querySelector("#edit-input"),a=i.querySelector("#cancel-btn"),l=i.querySelector("#save-btn");r.focus(),r instanceof HTMLInputElement?r.select():r.setSelectionRange(0,r.value.length),a==null||a.addEventListener("click",()=>{document.body.removeChild(i),o(null)}),l==null||l.addEventListener("click",()=>{const p=r.value.trim();document.body.removeChild(i),o(p)}),r instanceof HTMLInputElement&&r.addEventListener("keydown",p=>{if(p.key==="Enter"){const c=r.value.trim();document.body.removeChild(i),o(c)}}),i.addEventListener("click",p=>{p.target===i&&(document.body.removeChild(i),o(null))})})}showConfirmDialog(e){return new Promise(t=>{var s,r;const n=document.createElement("div");n.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      `,n.appendChild(o),document.body.appendChild(n);const i=()=>{document.body.removeChild(n),t()};(s=o.querySelector("#alert-ok"))==null||s.addEventListener("click",i),n.addEventListener("click",r=>{r.target===n&&i()})})}updateNavHighlight(){if(!this.container)return;this.container.querySelectorAll(".nav-item").forEach(t=>{const n=t;n.dataset.page===this.currentPage?n.classList.add("active"):n.classList.remove("active")})}bindEvents(){if(!this.container)return;const e=this.container.querySelector("#admin-login-form");if(e){e.addEventListener("submit",async u=>{u.preventDefault(),u.stopPropagation();const h=this.container.querySelector("#admin-username"),g=this.container.querySelector("#admin-password"),y=(h==null?void 0:h.value)||"",w=(g==null?void 0:g.value)||"";console.log("Login attempt with username:",y);try{const{DatabaseService:A}=await Promise.resolve().then(()=>E),_=await A.login(y,w);console.log("Login successful (database auth)"),this.isAuthenticated=!0,this.container.innerHTML=this.renderAdminUI(),await this.updatePageContent(),this.bindEvents()}catch(A){console.error("Login error:",A),this.showAlertDialog("登入時發生錯誤，請稍後再試").then(()=>{g.value="",g.focus()})}});const c=this.container.querySelector("#admin-username");c&&setTimeout(()=>{c.focus()},100)}setTimeout(()=>{const c=this.container.querySelectorAll(".nav-item");if(console.log("Binding nav items, found:",c.length),c.length===0&&this.isAuthenticated){console.warn("Nav items not found, retrying..."),setTimeout(()=>this.bindEvents(),100);return}c.forEach((u,h)=>{console.log(`Binding nav item ${h}:`,u.dataset.page);const g=u.cloneNode(!0);u.parentNode.replaceChild(g,u),g.addEventListener("click",async()=>{const y=g.dataset.page;console.log("Nav item clicked:",y),y&&y!==this.currentPage&&(this.currentPage=y,await this.updatePageContent(),this.updateNavHighlight())})})},50);const t=this.container.querySelector("#admin-logout");t&&t.addEventListener("click",()=>{this.isAuthenticated=!1,this.container.innerHTML=this.renderLoginUI(),this.bindEvents()});const n=this.container.querySelector("#telegram-settings-form");n&&n.addEventListener("submit",async c=>{c.preventDefault(),c.stopPropagation();const u=this.container.querySelector("#telegram-enabled"),h=(u==null?void 0:u.checked)||!1;this.setTelegramEnabled(h),alert(`Telegram 通知已${h?"啟用":"停用"}`),await this.updatePageContent()});const o=this.container.querySelector("#change-password-form");o&&o.addEventListener("submit",async c=>{c.preventDefault(),c.stopPropagation();const u=this.container.querySelector("#new-password"),h=(u==null?void 0:u.value)||"";if(h.length<4){alert("密碼長度至少 4 個字元");return}f.saveAdminPassword(h),alert("密碼已更新"),await this.updatePageContent()});const i=this.container.querySelector("#ip-whitelist-form");i&&i.addEventListener("submit",async c=>{c.preventDefault(),c.stopPropagation();const u=this.container.querySelector("#ip-list"),g=((u==null?void 0:u.value)||"").split(`
`).map(y=>y.trim()).filter(y=>y.length>0);this.saveIPWhitelist(g),alert(`已更新 IP 白名單（${g.length} 個 IP）`),await this.updatePageContent()});const s=this.container.querySelector("#api-config-form");s&&s.addEventListener("submit",c=>{var M,O,T,L,z,q;c.preventDefault(),c.stopPropagation();const u=((M=this.container.querySelector("#llm-endpoint"))==null?void 0:M.value)||"",h=((O=this.container.querySelector("#llm-api-key"))==null?void 0:O.value)||"",g=((T=this.container.querySelector("#llm-deployment"))==null?void 0:T.value)||"",y=((L=this.container.querySelector("#embed-endpoint"))==null?void 0:L.value)||"",w=((z=this.container.querySelector("#embed-api-key"))==null?void 0:z.value)||"",A=((q=this.container.querySelector("#embed-deployment"))==null?void 0:q.value)||"",_={azureOpenAI:{endpoint:u,apiKey:h,deployment:g,embeddingDeployment:A},llmAPI:{endpoint:u,apiKey:h,deployment:g},embeddingAPI:{endpoint:y,apiKey:w,deployment:A}};f.saveConfig(_),alert("API 設定已儲存")});const r=this.container.querySelector("#agent-tool-config-form");r&&r.addEventListener("submit",async c=>{var y,w;c.preventDefault(),c.stopPropagation();const u=((y=this.container.querySelector("#manual-index-enabled"))==null?void 0:y.checked)||!1,h=((w=this.container.querySelector("#frontend-pages-enabled"))==null?void 0:w.checked)||!1,g=f.loadAgentToolConfig();g&&(g.manualIndex.enabled=u,g.frontendPages.enabled=h,f.saveAgentToolConfig(g),alert("Agent 設定已儲存"),await this.updatePageContent())});const a=this.container.querySelector("#sql-plugin-config-form");a&&a.addEventListener("submit",async c=>{var T,L,z,q,U,j,H,B;c.preventDefault(),c.stopPropagation();const u=((T=this.container.querySelector("#sql-plugin-enabled"))==null?void 0:T.checked)||!1,h=parseInt(((L=this.container.querySelector("#sql-plugin-priority"))==null?void 0:L.value)||"5"),g=((z=this.container.querySelector("#sql-api-endpoint"))==null?void 0:z.value)||"",y=((q=this.container.querySelector("#sql-connection-id"))==null?void 0:q.value)||"",w=((U=this.container.querySelector("#sql-search-table"))==null?void 0:U.value)||"knowledge_base",A=((j=this.container.querySelector("#sql-title-column"))==null?void 0:j.value)||"title",_=((H=this.container.querySelector("#sql-content-column"))==null?void 0:H.value)||"content",M=((B=this.container.querySelector("#sql-url-column"))==null?void 0:B.value)||"url",O={enabled:u,priority:h,apiEndpoint:g,connectionId:y,searchTable:w,titleColumn:A,contentColumn:_,urlColumn:M};localStorage.setItem("sm_sql_plugin_config",JSON.stringify(O)),alert("SQL Plugin 設定已儲存"),await this.updatePageContent()});const l=this.container.querySelector("#sql-connection-form");l&&l.addEventListener("submit",async c=>{var g,y;c.preventDefault(),c.stopPropagation();const u=((g=this.container.querySelector("#sql-conn-name"))==null?void 0:g.value)||"",h=(y=this.container.querySelector("#sql-conn-type"))==null?void 0:y.value;if(!u){alert("請輸入連接名稱");return}try{k.create({name:u,type:h,host:"localhost",port:3306,database:"mydb",username:"user",password:"password",queryTemplate:"SELECT * FROM {table} WHERE {conditions}",resultMapping:{titleField:"title",contentField:"content",urlField:"url"}}),alert("SQL 連接已新增"),await this.updatePageContent()}catch(w){console.error("Error creating SQL connection:",w),alert("新增失敗")}}),this.container.querySelectorAll(".delete-sql-connection").forEach(c=>{c.addEventListener("click",async()=>{const u=c.dataset.id;if(u&&confirm("確定要刪除這個連接嗎？"))try{k.delete(u),alert("連接已刪除"),await this.updatePageContent()}catch(h){console.error("Error deleting SQL connection:",h),alert("刪除失敗")}})})}renderAdminUI(){return`
