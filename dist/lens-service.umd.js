...
`).map(g=>g.trim()).filter(g=>g.length>0);this.saveIPWhitelist(u),alert(`已更新 IP 白名單（${u.length} 個 IP）`),await this.updatePageContent()});const i=this.container.querySelector("#api-config-form");i&&i.addEventListener("submit",c=>{var d,h,u,g,b,y;c.preventDefault(),c.stopPropagation(),(d=this.container.querySelector("#llm-endpoint"))!=null&&d.value,(h=this.container.querySelector("#llm-api-key"))!=null&&h.value,(u=this.container.querySelector("#llm-deployment"))!=null&&u.value,(g=this.container.querySelector("#embed-endpoint"))!=null&&g.value,(b=this.container.querySelector("#embed-api-key"))!=null&&b.value,(y=this.container.querySelector("#embed-deployment"))!=null&&y.value,alert("API 設定已儲存")});const r=this.container.querySelector("#agent-tool-config-form");r&&r.addEventListener("submit",async c=>{var d,h;c.preventDefault(),c.stopPropagation(),(d=this.container.querySelector("#manual-index-enabled"))!=null&&d.checked,(h=this.container.querySelector("#frontend-pages-enabled"))!=null&&h.checked,alert("Agent 設定已儲存"),await this.updatePageContent()});const a=this.container.querySelector("#sql-plugin-config-form");a&&a.addEventListener("submit",async c=>{var x,M,D,q,T,_,O,j;c.preventDefault(),c.stopPropagation();const d=((x=this.container.querySelector("#sql-plugin-enabled"))==null?void 0:x.checked)||!1,h=parseInt(((M=this.container.querySelector("#sql-plugin-priority"))==null?void 0:M.value)||"5"),u=((D=this.container.querySelector("#sql-api-endpoint"))==null?void 0:D.value)||"",g=((q=this.container.querySelector("#sql-connection-id"))==null?void 0:q.value)||"",b=((T=this.container.querySelector("#sql-search-table"))==null?void 0:T.value)||"knowledge_base",y=((_=this.container.querySelector("#sql-title-column"))==null?void 0:_.value)||"title",S=((O=this.container.querySelector("#sql-content-column"))==null?void 0:O.value)||"content",z=((j=this.container.querySelector("#sql-url-column"))==null?void 0:j.value)||"url",P={enabled:d,priority:h,apiEndpoint:u,connectionId:g,searchTable:b,titleColumn:y,contentColumn:S,urlColumn:z};localStorage.setItem("sm_sql_plugin_config",JSON.stringify(P)),alert("SQL Plugin 設定已儲存"),await this.updatePageContent()});const l=this.container.querySelector("#sql-connection-form");l&&l.addEventListener("submit",async c=>{var h,u;c.preventDefault(),c.stopPropagation();const d=((h=this.container.querySelector("#sql-conn-name"))==null?void 0:h.value)||"";if((u=this.container.querySelector("#sql-conn-type"))==null||u.value,!d){alert("請輸入連接名稱");return}try{alert("SQL 連接已新增"),await this.updatePageContent()}catch(g){console.error("Error creating SQL connection:",g),alert("新增失敗")}}),this.container.querySelectorAll(".delete-sql-connection").forEach(c=>{c.addEventListener("click",async()=>{if(c.dataset.id&&confirm("確定要刪除這個連接嗎？"))try{alert("連接已刪除"),await this.updatePageContent()}catch(h){console.error("Error deleting SQL connection:",h),alert("刪除失敗")}})})}renderAdminUI(){return`
...
...
`)}),a.map(l=>({chunk:l.chunk,context:l.context,score:l.score}))}static clearCache(){this.cache=null}}m(E,"cache",null),m(E,"CACHE_DURATION",36e5),m(E,"CHUNK_SIZE",500),m(E,"CHUNK_OVERLAP",100);const K=Object.freeze(Object.defineProperty({__proto__:null,LlmsTxtService:E},Symbol.toStringTag,{value:"Module"}));return L});
