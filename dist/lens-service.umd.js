(function(m,w){typeof exports=="object"&&typeof module<"u"?module.exports=w():typeof define=="function"&&define.amd?define(w):(m=typeof globalThis<"u"?globalThis:m||self,m.LensService=w())})(this,function(){"use strict";var N=Object.defineProperty;var Q=(m,w,C)=>w in m?N(m,w,{enumerable:!0,configurable:!0,writable:!0,value:C}):m[w]=C;var b=(m,w,C)=>Q(m,typeof w!="symbol"?w+"":w,C);const m={container:`
  ...
  `};class w{constructor(e="33.33%",t="right"){b(this,"container");b(this,"overlay");b(this,"panel");b(this,"isOpen",!1);b(this,"width");b(this,"position");b(this,"capturedImage",null);b(this,"capturedText",null);b(this,"onSendMessage");b(this,"onSelectRule");b(this,"onClose");b(this,"onOpen");this.width=e,this.position=t,this.container=this.createContainer(),this.overlay=this.createOverlay(),this.panel=this.createPanel()}createContainer(){const e=document.createElement("div");return e.id="sm-container",e.style.cssText=m.container,e}createOverlay(){const e=document.createElement("div");return e.style.cssText=m.overlay,e.style.display="none",e.addEventListener("click",()=>this.close()),e}createPanel(){const e=document.createElement("div");return e.style.cssText=m.panel,e.style.width=this.width,this.position==="right"?(e.style.right=`-${this.width}`,e.style.left="auto"):(e.style.left=`-${this.width}`,e.style.right="auto"),e.innerHTML=`
    ...
    `,this.bindEvents(e),e}bindEvents(e){var n,i,r,s,a;(n=e.querySelector("#sm-close-btn"))==null||n.addEventListener("click",()=>{this.close()});const t=e.querySelector("#sm-send-btn");t?(console.log("✅ Send button found, binding click event"),t.addEventListener("click",c=>{console.log("🔥 Send button clicked via addEventListener!"),c.preventDefault(),c.stopPropagation(),this.handleSend()}),t.onclick=c=>{console.log("🔥 Send button clicked via onclick!"),c.preventDefault(),c.stopPropagation(),this.handleSend()},e.addEventListener("click",c=>{(c.target.id==="sm-send-btn"||c.target.closest("#sm-send-btn"))&&(console.log("🔥 Send button clicked via delegation!"),c.preventDefault(),c.stopPropagation(),this.handleSend())})):console.error("❌ Send button not found!");const o=e.querySelector("#sm-input");o?(console.log("✅ Input field found, binding events"),o.addEventListener("keypress",c=>{c.key==="Enter"&&(console.log("🔥 Enter key pressed in input"),this.handleSend())}),o.addEventListener("input",c=>{console.log("🔥 Input event:",c.target.value)}),o.addEventListener("focus",()=>{console.log("🔥 Input focused")}),o.addEventListener("blur",()=>{console.log("🔥 Input blurred")})):console.error("❌ Input field not found!"),(i=e.querySelector("#sm-chat-tab"))==null||i.addEventListener("click",()=>{this.showView("chat")}),(r=e.querySelector("#sm-refresh-btn"))==null||r.addEventListener("click",()=>{this.clearMessages()}),(s=e.querySelector("#sm-history-btn"))==null||s.addEventListener("click",()=>{this.showHistory()}),(a=e.querySelector("#sm-remove-image"))==null||a.addEventListener("click",()=>{this.clearCapturedImage()})}handleSend(){const e=this.panel.querySelector("#sm-input"),t=e.value.trim();(t||this.capturedImage)&&this.onSendMessage&&(this.onSendMessage(t,this.capturedImage||void 0,this.capturedText||void 0),e.value="",this.clearCapturedImage())}showView(e){const t=this.panel.querySelector("#sm-chat-view"),o=this.panel.querySelector("#sm-chat-tab");e==="chat"&&(t.style.display="flex",o.style.cssText=m.tabButton+"; "+m.tabButtonActive)}addMessage(e){const t=this.panel.querySelector("#sm-messages");if(!t)return;const o=document.createElement("div");if(o.style.cssText=e.role==="user"?m.userMessage:m.assistantMessage,e.role==="assistant"?o.innerHTML=e.content:o.textContent=e.content,e.sources&&e.sources.length>0){const n=document.createElement("div");n.style.cssText=m.sources,n.innerHTML="<strong>參考來源：</strong><br>",e.sources.forEach((i,r)=>{const s=document.createElement("a");s.href=i.url,s.target="_blank",s.textContent=`[${r+1}] ${i.title}`,s.style.cssText=m.sourceLink,n.appendChild(s),n.appendChild(document.createElement("br"))}),o.appendChild(n)}t.appendChild(o),setTimeout(()=>{t.scrollTop=t.scrollHeight},10)}setRules(e,t){}clearMessages(){const e=this.panel.querySelector("#sm-messages");e&&(e.innerHTML="")}async showHistory(){try{const{DatabaseService:e}=await Promise.resolve().then(()=>x),t=await e.getConversations();this.showHistoryView(t)}catch(e){console.error("Failed to load history:",e),alert("載入歷史記錄失敗")}}showHistoryView(e){const t=this.panel.querySelector("#sm-chat-view");if(console.log("📋 showHistoryView called with",e.length,"conversations"),console.log("📋 chatView:",t),!t){console.error("❌ chatView not found");return}t.style.display="none";let o=this.panel.querySelector("#sm-history-view");if(!o){o=document.createElement("div"),o.id="sm-history-view",o.style.cssText=m.chatView;const r=t.parentElement;if(console.log("📋 parent element:",r),r)r.appendChild(o),console.log("✅ History view created and appended");else{console.error("❌ Parent element not found");return}}if(o.style.display="flex",o.style.flexDirection="column",console.log("✅ History view display set to flex"),!Array.isArray(e)||e.length===0)o.innerHTML=`
      ...
      `;else{const r=e.map(s=>{let a=[];try{a=typeof s.messages=="string"?JSON.parse(s.messages):s.messages}catch{a=[]}const c=Array.isArray(a)?a.length:0,h=new Date(s.created_at).toLocaleString("zh-TW",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}),l=s.conversation_id||s.id||"unknown",d=l.toString().slice(-8);return`
    ...
    `}showEditDialog(e,t,o=!1){return new Promise(n=>{const i=document.createElement("div");i.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      `,document.body.appendChild(i);const s=i.querySelector("#edit-input"),a=i.querySelector("#cancel-btn"),c=i.querySelector("#save-btn");s.focus(),s instanceof HTMLInputElement?s.select():s.setSelectionRange(0,s.value.length),a==null||a.addEventListener("click",()=>{document.body.removeChild(i),n(null)}),c==null||c.addEventListener("click",()=>{const h=s.value.trim();document.body.removeChild(i),n(h)}),s instanceof HTMLInputElement&&s.addEventListener("keydown",h=>{if(h.key==="Enter"){const l=s.value.trim();document.body.removeChild(i),n(l)}}),i.addEventListener("click",h=>{h.target===i&&(document.body.removeChild(i),n(null))})})}showConfirmDialog(e){return new Promise(t=>{var r,s;const o=document.createElement("div");o.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      `,o.appendChild(n),document.body.appendChild(o);const i=a=>{document.body.removeChild(o),t(a)};(r=n.querySelector("#confirm-ok"))==null||r.addEventListener("click",()=>i(!0)),(s=n.querySelector("#confirm-cancel"))==null||s.addEventListener("click",()=>i(!1)),o.addEventListener("click",a=>{a.target===o&&i(!1)})})}showAlertDialog(e){return new Promise(t=>{var r;const o=document.createElement("div");o.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      `,o.appendChild(n),document.body.appendChild(o);const i=()=>{document.body.removeChild(o),t()};(r=n.querySelector("#alert-ok"))==null||r.addEventListener("click",i),o.addEventListener("click",s=>{s.target===o&&i()})})}updateNavHighlight(){if(!this.container)return;this.container.querySelectorAll(".nav-item").forEach(t=>{const o=t,n=o.dataset.page===this.currentPage;o.style.background=n?"#ede9fe":"transparent",o.style.color=n?"#7c3aed":"#4b5563",o.style.fontWeight=n?"600":"500",n?o.classList.add("active"):o.classList.remove("active")})}bindEvents(){if(!this.container)return;const e=this.container.querySelector("#admin-login-form");if(e){e.addEventListener("submit",async d=>{d.preventDefault(),d.stopPropagation();const p=this.container.querySelector("#admin-username"),u=this.container.querySelector("#admin-password"),g=(p==null?void 0:p.value)||"",f=(u==null?void 0:u.value)||"";console.log("Login attempt with username:",g);try{const{DatabaseService:y}=await Promise.resolve().then(()=>x),v=await y.validateAdmin(g,f);console.log("Login successful (database auth)"),this.isAuthenticated=!0,this.container.innerHTML=this.renderAdminUI(),await this.updatePageContent(),this.bindEvents()}catch(y){console.error("Login error:",y),this.showAlertDialog("登入時發生錯誤，請稍後再試").then(()=>{u.value="",u.focus()})}});const l=this.container.querySelector("#admin-username");l&&setTimeout(()=>{l.focus()},100)}setTimeout(()=>{const l=this.container.querySelectorAll(".nav-item");if(console.log("Binding nav items, found:",l.length),l.length===0&&this.isAuthenticated){console.warn("Nav items not found, retrying..."),setTimeout(()=>this.bindEvents(),100);return}l.forEach((d,p)=>{console.log(`Binding nav item ${p}:`,d.dataset.page);const u=d.cloneNode(!0);d.parentNode.replaceChild(u,d),u.addEventListener("click",async()=>{const g=u.dataset.page;console.log("Nav item clicked:",g),g&&g!==this.currentPage&&(this.currentPage=g,await this.updatePageContent(),this.updateNavHighlight())})})},50);const t=this.container.querySelector("#admin-logout");t&&t.addEventListener("click",()=>{this.isAuthenticated=!1,this.container.innerHTML=this.renderLoginUI(),this.bindEvents()});const o=this.container.querySelector("#telegram-settings-form");o&&o.addEventListener("submit",async l=>{l.preventDefault(),l.stopPropagation();const d=this.container.querySelector("#telegram-enabled"),p=(d==null?void 0:d.checked)||!1;this.setTelegramEnabled(p),alert(`Telegram 通知已${p?"啟用":"停用"}`),await this.updatePageContent()});const n=this.container.querySelector("#change-password-form");n&&n.addEventListener("submit",async l=>{l.preventDefault(),l.stopPropagation();const d=this.container.querySelector("#new-password");if(((d==null?void 0:d.value)||"").length<4){alert("密碼長度至少 4 個字元");return}alert("密碼已更新"),await this.updatePageContent()});const i=this.container.querySelector("#ip-whitelist-form");i&&i.addEventListener("submit",async l=>{l.preventDefault(),l.stopPropagation();const d=this.container.querySelector("#ip-list"),u=((d==null?void 0:d.value)||"").split(`
`).map(g=>g.trim()).filter(g=>g.length>0);this.saveIPWhitelist(u),alert(`已更新 IP 白名單（${u.length} 個 IP）`),await this.updatePageContent()});const r=this.container.querySelector("#api-config-form");r&&r.addEventListener("submit",l=>{var d,p,u,g,f,y;l.preventDefault(),l.stopPropagation(),(d=this.container.querySelector("#llm-endpoint"))!=null&&d.value,(p=this.container.querySelector("#llm-api-key"))!=null&&p.value,(u=this.container.querySelector("#llm-deployment"))!=null&&u.value,(g=this.container.querySelector("#embed-endpoint"))!=null&&g.value,(f=this.container.querySelector("#embed-api-key"))!=null&&f.value,(y=this.container.querySelector("#embed-deployment"))!=null&&y.value,alert("API 設定已儲存")});const s=this.container.querySelector("#agent-tool-config-form");s&&s.addEventListener("submit",async l=>{var d,p;l.preventDefault(),l.stopPropagation(),(d=this.container.querySelector("#manual-index-enabled"))!=null&&d.checked,(p=this.container.querySelector("#frontend-pages-enabled"))!=null&&p.checked,alert("Agent 設定已儲存"),await this.updatePageContent()});const a=this.container.querySelector("#sql-plugin-config-form");a&&a.addEventListener("submit",async l=>{var E,L,P,q,D,M,T,_;l.preventDefault(),l.stopPropagation();const d=((E=this.container.querySelector("#sql-plugin-enabled"))==null?void 0:E.checked)||!1,p=parseInt(((L=this.container.querySelector("#sql-plugin-priority"))==null?void 0:L.value)||"5"),u=((P=this.container.querySelector("#sql-api-endpoint"))==null?void 0:P.value)||"",g=((q=this.container.querySelector("#sql-connection-id"))==null?void 0:q.value)||"",f=((D=this.container.querySelector("#sql-search-table"))==null?void 0:D.value)||"knowledge_base",y=((M=this.container.querySelector("#sql-title-column"))==null?void 0:M.value)||"title",v=((T=this.container.querySelector("#sql-content-column"))==null?void 0:T.value)||"content",z=((_=this.container.querySelector("#sql-url-column"))==null?void 0:_.value)||"url",B={enabled:d,priority:p,apiEndpoint:u,connectionId:g,searchTable:f,titleColumn:y,contentColumn:v,urlColumn:z};localStorage.setItem("sm_sql_plugin_config",JSON.stringify(B)),alert("SQL Plugin 設定已儲存"),await this.updatePageContent()});const c=this.container.querySelector("#sql-connection-form");c&&c.addEventListener("submit",async l=>{var p,u;l.preventDefault(),l.stopPropagation();const d=((p=this.container.querySelector("#sql-conn-name"))==null?void 0:p.value)||"";if((u=this.container.querySelector("#sql-conn-type"))==null||u.value,!d){alert("請輸入連接名稱");return}try{alert("SQL 連接已新增"),await this.updatePageContent()}catch(g){console.error("Error creating SQL connection:",g),alert("新增失敗")}}),this.container.querySelectorAll(".delete-sql-connection").forEach(l=>{l.addEventListener("click",async()=>{if(l.dataset.id&&confirm("確定要刪除這個連接嗎？"))try{alert("連接已刪除"),await this.updatePageContent()}catch(p){console.error("Error deleting SQL connection:",p),alert("刪除失敗")}})})}renderAdminUI(){return`
    ...
    `,document.body.appendChild(o);const n=o.querySelector("#edit-index-form"),i=o.querySelector("#cancel-edit-btn");n.addEventListener("submit",async r=>{r.preventDefault();const s=o.querySelector("#edit-index-name").value;o.querySelector("#edit-index-description").value;const a=o.querySelector("#edit-index-content").value;if(!s||!a){await this.showAlertDialog("請填寫名稱和內容");return}try{await k.update(e,{title:s,content:a,url:""}),await this.showAlertDialog("索引已更新"),document.body.removeChild(o),await this.updatePageContent()}catch(c){await this.showAlertDialog(`更新失敗：${c instanceof Error?c.message:"未知錯誤"}`)}}),i.addEventListener("click",()=>{document.body.removeChild(o)}),o.addEventListener("click",r=>{r.target===o&&document.body.removeChild(o)})}async showAddIndexModal(){const e=document.createElement("div");e.style.cssText=`
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
    ...
    `,document.body.appendChild(e);const t=e.querySelector("#add-index-form"),o=e.querySelector("#cancel-add-btn");t.addEventListener("submit",async n=>{n.preventDefault();const i=e.querySelector("#add-index-name").value;e.querySelector("#add-index-description").value;const r=e.querySelector("#add-index-url").value,s=e.querySelector("#add-index-content").value;if(!i||!s){await this.showAlertDialog("請填寫名稱和內容");return}try{await k.create({title:i,content:s,url:r||void 0}),await this.showAlertDialog("索引已新增"),document.body.removeChild(e),await this.updatePageContent()}catch(a){await this.showAlertDialog(`新增失敗：${a instanceof Error?a.message:"未知錯誤"}`)}}),o.addEventListener("click",()=>{document.body.removeChild(e)}),e.addEventListener("click",n=>{n.target===e&&document.body.removeChild(e)})}async showDeleteConfirmDialog(e){const t=await k.getById(e);if(!t){await this.showAlertDialog("找不到該索引");return}if(await this.showConfirmDialog(`確定要刪除索引「${t.title||t.name||"未命名"}」嗎？此操作無法復原。`))try{await k.delete(e),await this.showAlertDialog("索引已刪除"),await this.updatePageContent()}catch(n){await this.showAlertDialog(`刪除失敗：${n instanceof Error?n.message:"未知錯誤"}`)}}async renderConversations(){try{const{CustomerServiceManager:e}=await Promise.resolve().then(()=>A),t=await e.getAllConversations();return`
          ...
          `:`
            <div style="overflow-x: auto;">
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                  ${t.slice().reverse().map(o=>{const n=o.conversation_id||o.conversationId||o.id,i=o.user_id||o.userId||"undefined",r=Array.isArray(o.messages)?o.messages:[],s=o.status||"active",a=o.created_at||o.createdAt||o.startedAt;return`
      ...
      `}}async renderSystemSettings(){let e={},t=[];try{const{DatabaseService:i}=await Promise.resolve().then(()=>x),[r,s]=await Promise.all([i.getSettings().catch(()=>({})),i.getAdminUsers().catch(()=>[])]);e=r,t=s}catch(i){console.error("Failed to load system settings:",i)}const o=e.default_reply||"",n=e.system_prompt||"";return`
    ...
    `,document.body.appendChild(e);const t=e.querySelector("#add-admin-user-form"),o=e.querySelector("#cancel-add-admin-btn");t.addEventListener("submit",async n=>{n.preventDefault();const i=e.querySelector("#add-admin-username").value,r=e.querySelector("#add-admin-password").value,s=e.querySelector("#add-admin-email").value;try{const{DatabaseService:a}=await Promise.resolve().then(()=>x);await a.createAdminUser(i,r,s),document.body.removeChild(e),await this.showAlertDialog("管理員帳號已新增"),await this.updatePageContent()}catch(a){await this.showAlertDialog(`新增失敗：${a instanceof Error?a.message:"未知錯誤"}`)}}),o.addEventListener("click",()=>{document.body.removeChild(e)}),e.addEventListener("click",n=>{n.target===e&&document.body.removeChild(e)})}async showConversationModal(e){try{const{CustomerServiceManager:t}=await Promise.resolve().then(()=>A),o=await t.getConversationById(e);if(!o){await this.showAlertDialog("找不到該對話記錄");return}const n=o.conversation_id||o.conversationId||o.id,i=o.user_id||o.userId||"undefined",r=Array.isArray(o.messages)?o.messages:[],s=o.status||"active",a=o.created_at||o.createdAt,c=o.updated_at||o.updatedAt,h=document.createElement("div");h.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      `,document.body.appendChild(h);const l=h.querySelector("#close-conversation-modal"),d=h.querySelector("#close-conversation-modal-btn"),p=h.querySelector("#send-customer-service-reply"),u=h.querySelector("#customer-service-reply"),g=()=>{document.body.removeChild(h)};l==null||l.addEventListener("click",g),d==null||d.addEventListener("click",g),p==null||p.addEventListener("click",async()=>{const f=u==null?void 0:u.value.trim();if(!f){await this.showAlertDialog("請輸入回覆內容");return}try{const{CustomerServiceManager:y}=await Promise.resolve().then(()=>A);await y.addCustomerServiceReply(e,f,"客服")?(await this.showAlertDialog("回覆已發送"),g(),await this.updatePageContent()):await this.showAlertDialog("發送失敗，請稍後再試")}catch(y){console.error("Failed to send reply:",y),await this.showAlertDialog(`發送失敗：${y instanceof Error?y.message:"未知錯誤"}`)}}),h.addEventListener("click",f=>{f.target===h&&g()})}catch(t){console.error("Error showing conversation modal:",t),await this.showAlertDialog("載入對話詳情失敗")}}}class H{constructor(){b(this,"config");b(this,"panel");b(this,"conversationState");b(this,"initialized",!1);b(this,"captureMode",!1);b(this,"adminPanel");b(this,"floatingIcon");b(this,"screenshotMode",!1);b(this,"hoverHandler",null);b(this,"mouseLeaveHandler",null)}async loadRulesFromSQL(){try{const e=await fetch("http://localhost:3002/rules");if(!e.ok)return console.log("No rules found in database, using empty array"),[];const t=await e.json();return Array.isArray(t)?t:[]}catch(e){return console.error("Failed to load rules from SQL:",e),[]}}async init(e){var o,n,i;if(this.initialized){console.warn("ServiceModuler already initialized");return}this.config=e,console.log("✅ Widget initializing");const t=e.telegram&&e.telegram.botToken&&e.telegram.chatId?e.telegram:void 0;window.SM_TELEGRAM_CONFIG=t,await this.loadRulesFromSQL(),this.panel=new w(((o=e.ui)==null?void 0:o.width)||"33.33%",((n=e.ui)==null?void 0:n.position)||"right"),this.panel.setCallbacks({onSendMessage:(r,s)=>this.handleSendMessage(r,s),onSelectRule:r=>this.handleSelectRule(r),onClose:()=>this.handleClose(),onOpen:()=>this.handleOpen()}),await this.loadConversationState(),this.adminPanel||(this.adminPanel=new j),window.location.pathname==="/lens-service"&&this.openAdminPanel(),this.bindGlobalKeyboardShortcuts(),((i=e.ui)==null?void 0:i.iconPosition)!==!1&&!this.isAdminPage()&&this.createFloatingIcon(),this.initialized=!0,e.debug&&console.log("ServiceModuler initialized",e)}bindGlobalKeyboardShortcuts(){document.addEventListener("keydown",e=>{var t,o;e.key&&e.key.toLowerCase()==="q"&&((t=this.panel)!=null&&t.isPanelOpen())?(console.log("🎯 Q key pressed, panel is open, enabling screenshot mode"),this.enableScreenshotMode()):e.key&&e.key.toLowerCase()==="q"&&console.log("🎯 Q key pressed, but panel is not open:",(o=this.panel)==null?void 0:o.isPanelOpen())}),document.addEventListener("keyup",e=>{e.key&&e.key.toLowerCase()==="q"&&this.disableScreenshotMode()}),document.addEventListener("click",e=>{var t;this.screenshotMode&&((t=this.panel)!=null&&t.isPanelOpen())&&(console.log("📸 Screenshot click detected"),e.preventDefault(),e.stopPropagation(),this.captureElementScreenshot(e.target))},!0)}open(){var e;if(!this.initialized){console.error("ServiceModuler not initialized. Call init() first.");return}(e=this.panel)==null||e.open()}close(){var e;(e=this.panel)==null||e.close()}async sendMessage(e,t){var n,i,r;if(!this.initialized||!this.panel){console.error("ServiceModuler not initialized");return}const o={role:"user",content:e||"請分析這張圖片",timestamp:Date.now()};(n=this.conversationState)==null||n.messages.push(o),this.panel.addMessage(o),this.saveConversationState();try{let s,a,c=!1;const h=((i=this.conversationState)==null?void 0:i.sessionId)||this.generateSessionId(),l=localStorage.getItem("lens_service_user_id")||"default_user";if(t)s=await this.processImageMessage(e,t);else{const p=await this.processTextMessage(e,h,l);s=p.response,a=p.sources,c=p.needsHumanReply,c&&await this.sendTelegramNotification(e,h)}const d={role:"assistant",content:s,timestamp:Date.now(),sources:a};(r=this.conversationState)==null||r.messages.push(d),this.panel.addMessage(d),this.saveConversationState(),await this.saveConversationToDatabase(h,l)}catch(s){console.error("Error processing message:",s);const a={role:"assistant",content:`抱歉，發生錯誤：${s instanceof Error?s.message:"未知錯誤"}`,timestamp:Date.now()};this.panel.addMessage(a)}}async processTextMessage(e,t,o){var n,i,r,s;try{const{DatabaseService:a}=await Promise.resolve().then(()=>x);await a.initializePool();const c=await a.getSetting("system_prompt")||"你是一個專業的客服助手，請用繁體中文回答問題。",h=await a.getSetting("default_reply")||"很抱歉，我無法回答這個問題。請聯繫人工客服獲得更多幫助。",{ManualIndexService:l}=await Promise.resolve().then(()=>O),d=await l.search(e);if(console.log("🔍 Search results:",d),!d||d.length===0)return console.log("❌ No relevant content found, using default reply"),{response:h,sources:[],needsHumanReply:!0};if(!((i=(n=this.config)==null?void 0:n.azureOpenAI)!=null&&i.endpoint)||!((s=(r=this.config)==null?void 0:r.azureOpenAI)!=null&&s.apiKey))return console.warn("Azure OpenAI not configured, using default reply"),{response:h,sources:[],needsHumanReply:!0};const p=d.map(v=>`標題：${v.title||v.name}
內容：${v.content}`).join(`
...
截圖分析功能暫時停用`,role:"assistant",timestamp:Date.now()})}catch(r){console.error("❌ Failed to send screenshot to AI:",r),(i=this.panel)==null||i.addMessage({id:Date.now().toString(),content:"截圖分析失敗，請檢查 AI 服務配置。",role:"assistant",timestamp:Date.now()})}}generateSessionId(){return`sm_${Date.now()}_${Math.random().toString(36).substr(2,9)}`}setConversationId(e){this.conversationState&&(this.conversationState.sessionId=e)}}const $=new H;typeof window<"u"&&(window.LensService=$);class F{static async getAllConversations(){try{const{DatabaseService:e}=await Promise.resolve().then(()=>x);await e.initializePool();const t=await e.getConversations();return Array.isArray(t)?t:[]}catch(e){return console.error("Failed to load conversations:",e),[]}}static async getConversationById(e){try{const{DatabaseService:t}=await Promise.resolve().then(()=>x);return await t.initializePool(),await t.getConversation(e)}catch(t){return console.error("Failed to load conversation:",t),null}}static async addCustomerServiceReply(e,t,o="客服"){try{const{DatabaseService:n}=await Promise.resolve().then(()=>x);await n.initializePool();const i=await n.getConversation(e);if(!i)return!1;const r={id:Date.now().toString(),role:"assistant",content:t,timestamp:Date.now(),metadata:{isCustomerService:!0,agentName:o}};return i.messages.push(r),await n.saveConversation(e,i.user_id||"unknown",i.messages),!0}catch(n){return console.error("Failed to add customer service reply:",n),!1}}static async deleteConversation(e){try{return(await fetch(`http://localhost:3002/conversations/${e}`,{method:"DELETE"})).ok}catch(t){return console.error("Failed to delete conversation:",t),!1}}static async markConversationAsHandled(e){try{return(await fetch(`http://localhost:3002/conversations/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:"handled",handledAt:Date.now()})})).ok}catch(t){return console.error("Failed to mark conversation as handled:",t),!1}}static async getPendingConversationsCount(){try{return(await this.getAllConversations()).filter(t=>t.status==="active").length}catch(e){return console.error("Failed to get pending conversations count:",e),0}}}const A=Object.freeze(Object.defineProperty({__proto__:null,CustomerServiceManager:F},Symbol.toStringTag,{value:"Module"}));class U{static async getAllAdminUsers(){try{return(await S.getAdminUsers()).map(t=>({id:t.id.toString(),username:t.username,password:"",email:t.email,created_at:new Date(t.created_at).getTime(),is_active:!0}))}catch(e){return console.error("Failed to load admin users:",e),[]}}static async createAdminUser(e,t,o){try{return await S.createAdminUser(e,t,o),!0}catch(n){return console.error("Failed to create admin user:",n),!1}}static async updateAdminUser(e,t){try{return(await fetch(`http://localhost:3002/admin-users/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})).ok}catch(o){return console.error("Failed to update admin user:",o),!1}}static async deleteAdminUser(e){try{return(await fetch(`http://localhost:3002/admin-users/${e}`,{method:"DELETE"})).ok}catch(t){return console.error("Failed to delete admin user:",t),!1}}static async validateAdminLogin(e,t){try{const o=await S.validateAdmin(e,t);return o?{id:o.id.toString(),username:o.username,password:"",email:o.email,created_at:Date.now(),last_login:Date.now(),is_active:!0}:null}catch(o){return console.error("Failed to validate admin login:",o),null}}static async changePassword(e,t){return await this.updateAdminUser(e,{password:t})}}const R=Object.freeze(Object.defineProperty({__proto__:null,AdminUserManager:U},Symbol.toStringTag,{value:"Module"}));return $});
