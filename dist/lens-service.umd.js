...
`)}`}),r.push({role:"user",content:e});try{const c=await this.openAI.chatCompletion(r,((a=this.currentRule)==null?void 0:a.temperature)||.7,((d=this.currentRule)==null?void 0:d.maxTokens)||1e3),h=!c.includes("CANNOT_ANSWER");return{response:c.replace(/CANNOT_ANSWER/g,"").trim()||c,canAnswer:h}}catch(c){return console.error("Failed to generate response:",c),{response:"抱歉，系統暫時無法處理您的請求。",canAnswer:!1}}}getRecentQA(e,t){const n=[];let o="";for(let i=e.length-1;i>=0&&n.length<t;i--){const r=e[i];r.role==="assistant"&&o?(n.unshift(`Q: ${o}
...
`}),t}getSourceTypeName(e){return{"manual-index":"手動索引","frontend-page":"前端頁面",sitemap:"Sitemap",sql:"SQL 資料庫"}[e]||e}getRules(){return this.rules}getCurrentRule(){return this.currentRule}}const b={container:`
  ...
  `,tabButtonActive:`
    ...
    color: white;
    border-color: #6366f1;
  `};class V{constructor(e="33.33%",t="right"){l(this,"container");l(this,"overlay");l(this,"panel");l(this,"isOpen",!1);l(this,"width");l(this,"position");l(this,"capturedImage",null);l(this,"capturedText",null);l(this,"onSendMessage");l(this,"onSelectRule");l(this,"onClose");l(this,"onOpen");this.width=e,this.position=t,this.container=this.createContainer(),this.overlay=this.createOverlay(),this.panel=this.createPanel()}createContainer(){const e=document.createElement("div");return e.id="sm-container",e.style.cssText=b.container,e}createOverlay(){const e=document.createElement("div");return e.style.cssText=b.overlay,e.style.display="none",e.addEventListener("click",()=>this.close()),e}createPanel(){const e=document.createElement("div");return e.style.cssText=b.panel,e.style.width=this.width,this.position==="right"?(e.style.right=`-${this.width}`,e.style.left="auto"):(e.style.left=`-${this.width}`,e.style.right="auto"),e.innerHTML=`
...
`).trim()}searchRelevantSections(e,t=5){const n=this.extract(),o=e.toLowerCase().split(/\s+/);return n.sections.map(r=>{let s=r.relevance;const a=r.heading.toLowerCase();o.forEach(c=>{a.includes(c)&&(s+=5)});const d=r.content.toLowerCase();return o.forEach(c=>{const h=(d.match(new RegExp(c,"g"))||[]).length;s+=h*2}),{heading:r.heading,content:r.content,score:s}}).sort((r,s)=>s.score-r.score).slice(0,t)}extractText(e){const t=e.cloneNode(!0);return t.querySelectorAll("script, style, noscript").forEach(n=>n.remove()),t.textContent||""}extractKeywords(e,t=20){const n=e.toLowerCase().replace(/[^\w\s\u4e00-\u9fa5]/g," ").split(/\s+/).filter(i=>i.length>1),o=new Map;for(const i of n)o.set(i,(o.get(i)||0)+1);return Array.from(o.entries()).sort((i,r)=>r[1]-i[1]).slice(0,t).map(([i])=>i)}generateFingerprint(e,t=64){const n=this.extractKeywords(e,50),o=new Array(t).fill(0);for(const i of n){const r=this.simpleHash(i,t);for(let s=0;s<t;s++)r[s]===1?o[s]++:o[s]--}return o.map(i=>i>0?1:0)}simpleHash(e,t){let n=0;for(let i=0;i<e.length;i++)n=(n<<5)-n+e.charCodeAt(i),n=n&n;const o=new Array(t).fill(0);for(let i=0;i<t;i++)o[i]=n>>i&1;return o}}class C{static getAll(){const e=localStorage.getItem(this.STORAGE_KEY);if(!e)return[];try{return JSON.parse(e)}catch(t){return console.error("Failed to parse manual indexes:",t),[]}}static getById(e){return this.getAll().find(n=>n.id===e)||null}static create(e){const t=new T,n=t.extractKeywords(e.content),o=t.generateFingerprint(e.content),i={id:this.generateId(),name:e.name,description:e.description,content:e.content,keywords:n,fingerprint:o,metadata:e.metadata||{},createdAt:Date.now(),updatedAt:Date.now()},r=this.getAll();return r.push(i),this.saveAll(r),console.log("Created manual index:",i.id),i}static update(e,t){const n=this.getAll(),o=n.find(i=>i.id===e);if(!o)return null;if(t.name!==void 0&&(o.name=t.name),t.description!==void 0&&(o.description=t.description),t.metadata!==void 0&&(o.metadata=t.metadata),t.content!==void 0){o.content=t.content;const i=new T;o.keywords=i.extractKeywords(t.content),o.fingerprint=i.generateFingerprint(t.content)}return o.updatedAt=Date.now(),this.saveAll(n),console.log("Updated manual index:",e),o}static delete(e){const t=this.getAll(),n=t.filter(o=>o.id!==e);return n.length===t.length?!1:(this.saveAll(n),console.log("Deleted manual index:",e),!0)}static search(e,t=5){const n=this.getAll();if(n.length===0)return[];const o=new T,i=o.extractKeywords(e),r=o.generateFingerprint(e);return n.map(a=>{const d=this.calculateSimilarity(i,r,a.keywords,a.fingerprint);return{index:a,score:d}}).filter(a=>a.score>0).sort((a,d)=>d.score-a.score).slice(0,t)}static calculateSimilarity(e,t,n,o){const i=this.calculateKeywordScore(e,n),r=this.calculateFingerprintScore(t,o);return i*.5+r*.5}static calculateKeywordScore(e,t){return e.length===0||t.length===0?0:e.filter(o=>t.includes(o)).length/Math.max(e.length,t.length)}static calculateFingerprintScore(e,t){if(e.length===0||t.length===0)return 0;let n=0,o=0;for(let i=0;i<Math.max(e.length,t.length);i++){const r=e[i]||0,s=t[i]||0;r===1&&s===1&&n++,(r===1||s===1)&&o++}return o>0?n/o:0}static saveAll(e){localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}static generateId(){return"idx_"+Date.now().toString(36)+Math.random().toString(36).substring(2)}static clearAll(){localStorage.removeItem(this.STORAGE_KEY)}static exportToJSON(){const e=this.getAll();return JSON.stringify(e,null,2)}static importFromJSON(e){try{const t=JSON.parse(e);if(!Array.isArray(t))throw new Error("Invalid format: expected array");const o=[...this.getAll(),...t];return this.saveAll(o),console.log(`Imported ${t.length} manual indexes`),t.length}catch(t){throw console.error("Failed to import indexes:",t),t}}}l(C,"STORAGE_KEY","sm_manual_indexes");class k{static getAll(){const e=localStorage.getItem(this.STORAGE_KEY);if(!e)return[];try{return JSON.parse(e)}catch(t){return console.error("Failed to parse SQL connections:",t),[]}}static getById(e){return this.getAll().find(n=>n.id===e)||null}static create(e){const t={id:this.generateId(),name:e.name,type:e.type,enabled:!0,createdAt:new Date().toISOString(),config:{host:e.host,port:e.port,database:e.database,username:e.username,password:e.password},queryTemplate:e.queryTemplate,resultMapping:e.resultMapping},n=this.getAll();return n.push(t),this.saveAll(n),console.log("Created SQL connection:",t.id),t}static update(e,t){const n=this.getAll(),o=n.find(i=>i.id===e);return o?(t.name!==void 0&&(o.name=t.name),t.type!==void 0&&(o.type=t.type),t.enabled!==void 0&&(o.enabled=t.enabled),t.config!==void 0&&(o.config={...o.config,...t.config}),t.queryTemplate!==void 0&&(o.queryTemplate=t.queryTemplate),t.resultMapping!==void 0&&(o.resultMapping={...o.resultMapping,...t.resultMapping}),this.saveAll(n),console.log("Updated SQL connection:",e),o):null}static delete(e){const t=this.getAll(),n=t.filter(o=>o.id!==e);return n.length===t.length?!1:(this.saveAll(n),console.log("Deleted SQL connection:",e),!0)}static async testConnection(e,t){const n=this.getById(e);if(!n)throw new Error("Connection not found");try{return(await(await fetch(`${t}/sql/test`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:n.type,config:n.config})})).json()).success===!0}catch(o){return console.error("Failed to test connection:",o),!1}}static async query(e,t,n){const o=this.getById(e);if(!o||!o.enabled)return[];try{const i=o.queryTemplate.replace(/\{\{query\}\}/g,t),s=await(await fetch(`${n}/sql/query`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:o.type,config:o.config,sql:i})})).json();if(!s.success||!s.rows)throw new Error(s.error||"Query failed");return s.rows.map(a=>({title:a[o.resultMapping.titleField]||"",content:a[o.resultMapping.contentField]||"",url:o.resultMapping.urlField?a[o.resultMapping.urlField]:void 0}))}catch(i){return console.error("Failed to execute query:",i),[]}}static async search(e,t,n,o=5){const i=this.getAll().filter(a=>a.enabled),r=n&&n.length>0?i.filter(a=>n.includes(a.id)):i;if(r.length===0)return[];const s=[];for(const a of r)try{const d=await this.query(a.id,e,t);for(const c of d)s.push({...c,connectionName:a.name})}catch(d){console.error(`Failed to search connection ${a.name}:`,d)}return s.slice(0,o)}static getStats(){const e=this.getAll(),t={total:e.length,enabled:e.filter(n=>n.enabled).length,byType:{}};for(const n of e)t.byType[n.type]=(t.byType[n.type]||0)+1;return t}static saveAll(e){localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}static generateId(){return"sql_"+Date.now().toString(36)+Math.random().toString(36).substring(2)}static clearAll(){localStorage.removeItem(this.STORAGE_KEY)}static exportConfig(){const e=this.getAll().map(t=>({...t,config:{...t.config,password:"***"}}));return JSON.stringify(e,null,2)}}l(k,"STORAGE_KEY","sm_sql_connections");var U={};class W{constructor(){l(this,"container",null);l(this,"isOpen",!1);l(this,"isAuthenticated",!1);l(this,"currentPage","dashboard");this.init()}init(){this.handleRouteChange(),window.addEventListener("popstate",()=>this.handleRouteChange()),this.interceptHistory()}interceptHistory(){const e=history.pushState,t=history.replaceState;history.pushState=(...n)=>{e.apply(history,n),this.handleRouteChange()},history.replaceState=(...n)=>{t.apply(history,n),this.handleRouteChange()}}handleRouteChange(){const e=window.location.pathname;e==="/lens-service"||e.startsWith("/lens-service/")?this.open():this.isOpen&&this.close()}open(){if(!this.isOpen){if(!this.checkIPWhitelist()){alert("您的 IP 不在白名單中，無法訪問管理後台"),window.location.href="/";return}this.isOpen=!0,this.container=document.createElement("div"),this.container.id="lens-service-admin",this.container.style.cssText=`
    ...
    `}bindEvents(){if(!this.container)return;const e=this.container.querySelector("#admin-login-form");if(e){e.addEventListener("submit",async g=>{g.preventDefault(),g.stopPropagation();const u=this.container.querySelector("#admin-username"),f=this.container.querySelector("#admin-password"),v=(u==null?void 0:u.value)||"",y=(f==null?void 0:f.value)||"";console.log("Login attempt with username:",v),v==="lens"&&y==="1234"?(console.log("Login successful (local auth)"),this.isAuthenticated=!0,this.container.innerHTML=this.renderAdminUI(),this.bindEvents()):(alert("用戶名或密碼錯誤"),f.value="",f.focus())});const p=this.container.querySelector("#admin-username");p&&setTimeout(()=>{p.focus()},100)}this.container.querySelectorAll(".nav-item").forEach(p=>{p.addEventListener("click",()=>{const g=p.dataset.page;if(g){this.currentPage=g;const u=this.container.querySelector("#admin-content");u&&(u.innerHTML=this.renderPageContent()),this.container.innerHTML=this.renderAdminUI(),this.bindEvents()}})});const n=this.container.querySelector("#admin-logout");n&&n.addEventListener("click",()=>{this.isAuthenticated=!1,this.container.innerHTML=this.renderLoginUI(),this.bindEvents()});const o=this.container.querySelector("#telegram-settings-form");o&&o.addEventListener("submit",p=>{p.preventDefault(),p.stopPropagation();const g=this.container.querySelector("#telegram-enabled"),u=(g==null?void 0:g.checked)||!1;this.setTelegramEnabled(u),alert(`Telegram 通知已${u?"啟用":"停用"}`);const f=this.container.querySelector("#admin-content");f&&(f.innerHTML=this.renderPageContent(),this.bindEvents())});const i=this.container.querySelector("#change-password-form");i&&i.addEventListener("submit",p=>{p.preventDefault(),p.stopPropagation();const g=this.container.querySelector("#new-password"),u=(g==null?void 0:g.value)||"";if(u.length<4){alert("密碼長度至少 4 個字元");return}m.saveAdminPassword(u),alert("密碼已更新");const f=this.container.querySelector("#admin-content");f&&(f.innerHTML=this.renderPageContent(),this.bindEvents())});const r=this.container.querySelector("#ip-whitelist-form");r&&r.addEventListener("submit",p=>{p.preventDefault(),p.stopPropagation();const g=this.container.querySelector("#ip-list"),f=((g==null?void 0:g.value)||"").split(`
`).map(y=>y.trim()).filter(y=>y.length>0);this.saveIPWhitelist(f),alert(`已更新 IP 白名單（${f.length} 個 IP）`);const v=this.container.querySelector("#admin-content");v&&(v.innerHTML=this.renderPageContent(),this.bindEvents())});const s=this.container.querySelector("#add-manual-index-form");s&&s.addEventListener("submit",p=>{p.preventDefault(),p.stopPropagation();const g=this.container.querySelector("#index-name"),u=this.container.querySelector("#index-description"),f=this.container.querySelector("#index-content"),v=(g==null?void 0:g.value)||"",y=(u==null?void 0:u.value)||"",A=(f==null?void 0:f.value)||"";if(!v||!A){alert("請填寫名稱和內容");return}try{C.create({name:v,description:y,content:A}),alert("索引已新增");const E=this.container.querySelector("#admin-content");E&&(E.innerHTML=this.renderPageContent(),this.bindEvents())}catch(E){alert(`新增失敗：${E instanceof Error?E.message:"未知錯誤"}`)}}),this.container.querySelectorAll(".delete-index-btn").forEach(p=>{p.addEventListener("click",()=>{const g=p.dataset.id;if(g&&confirm("確定要刪除這個索引嗎？"))try{C.delete(g),alert("索引已刪除");const u=this.container.querySelector("#admin-content");u&&(u.innerHTML=this.renderPageContent(),this.bindEvents())}catch(u){alert(`刪除失敗：${u instanceof Error?u.message:"未知錯誤"}`)}})});const d=this.container.querySelector("#api-config-form");d&&d.addEventListener("submit",p=>{var N,R,L,_,q,M;p.preventDefault(),p.stopPropagation();const g=((N=this.container.querySelector("#llm-endpoint"))==null?void 0:N.value)||"",u=((R=this.container.querySelector("#llm-api-key"))==null?void 0:R.value)||"",f=((L=this.container.querySelector("#llm-deployment"))==null?void 0:L.value)||"",v=((_=this.container.querySelector("#embed-endpoint"))==null?void 0:_.value)||"",y=((q=this.container.querySelector("#embed-api-key"))==null?void 0:q.value)||"",A=((M=this.container.querySelector("#embed-deployment"))==null?void 0:M.value)||"",E={azureOpenAI:{endpoint:g,apiKey:u,deployment:f,embeddingDeployment:A},llmAPI:{endpoint:g,apiKey:u,deployment:f},embeddingAPI:{endpoint:v,apiKey:y,deployment:A}};m.saveConfig(E),alert("API 設定已儲存")});const c=this.container.querySelector("#agent-tool-config-form");c&&c.addEventListener("submit",p=>{var v,y;p.preventDefault(),p.stopPropagation();const g=((v=this.container.querySelector("#manual-index-enabled"))==null?void 0:v.checked)||!1,u=((y=this.container.querySelector("#frontend-pages-enabled"))==null?void 0:y.checked)||!1,f=m.loadAgentToolConfig();if(f){f.manualIndex.enabled=g,f.frontendPages.enabled=u,m.saveAgentToolConfig(f),alert("Agent 設定已儲存");const A=this.container.querySelector("#admin-content");A&&(A.innerHTML=this.renderPageContent(),this.bindEvents())}});const h=this.container.querySelector("#sql-plugin-config-form");h&&h.addEventListener("submit",p=>{var _,q,M,B,Q,H,Y,J;p.preventDefault(),p.stopPropagation();const g=((_=this.container.querySelector("#sql-plugin-enabled"))==null?void 0:_.checked)||!1,u=parseInt(((q=this.container.querySelector("#sql-plugin-priority"))==null?void 0:q.value)||"5"),f=((M=this.container.querySelector("#sql-api-endpoint"))==null?void 0:M.value)||"",v=((B=this.container.querySelector("#sql-connection-id"))==null?void 0:B.value)||"",y=((Q=this.container.querySelector("#sql-search-table"))==null?void 0:Q.value)||"knowledge_base",A=((H=this.container.querySelector("#sql-title-column"))==null?void 0:H.value)||"title",E=((Y=this.container.querySelector("#sql-content-column"))==null?void 0:Y.value)||"content",N=((J=this.container.querySelector("#sql-url-column"))==null?void 0:J.value)||"url",R={enabled:g,priority:u,apiEndpoint:f,connectionId:v,searchTable:y,titleColumn:A,contentColumn:E,urlColumn:N};localStorage.setItem("sm_sql_plugin_config",JSON.stringify(R)),alert("SQL Plugin 設定已儲存");const L=this.container.querySelector("#admin-content");L&&(L.innerHTML=this.renderPageContent(),this.bindEvents())});const w=this.container.querySelector("#sql-connection-form");w&&w.addEventListener("submit",p=>{var f,v;p.preventDefault(),p.stopPropagation();const g=((f=this.container.querySelector("#sql-conn-name"))==null?void 0:f.value)||"",u=(v=this.container.querySelector("#sql-conn-type"))==null?void 0:v.value;if(!g){alert("請輸入連接名稱");return}try{k.create({name:g,type:u,host:"localhost",port:3306,database:"mydb",username:"user",password:"password",queryTemplate:"SELECT * FROM {table} WHERE {conditions}",resultMapping:{titleField:"title",contentField:"content",urlField:"url"}}),alert("SQL 連接已新增");const y=this.container.querySelector("#admin-content");y&&(y.innerHTML=this.renderPageContent(),this.bindEvents())}catch(y){console.error("Error creating SQL connection:",y),alert("新增失敗")}}),this.container.querySelectorAll(".delete-sql-connection").forEach(p=>{p.addEventListener("click",()=>{const g=p.dataset.id;if(g&&confirm("確定要刪除這個連接嗎？"))try{k.delete(g),alert("連接已刪除");const u=this.container.querySelector("#admin-content");u&&(u.innerHTML=this.renderPageContent(),this.bindEvents())}catch(u){console.error("Error deleting SQL connection:",u),alert("刪除失敗")}})})}renderAdminUI(){return`
    ...
    `}renderManualIndex(){const e=C.getAll();return`
    ...
    `}renderAgentAndAPI(){var n,o,i,r,s,a,d,c,h,w,S,p;const e=m.loadConfig()||{},t=m.loadAgentToolConfig();return`
    ...
    `}renderSystemSettings(){const e=m.loadAdminPassword(),t=this.getIPWhitelist(),n=this.getTelegramEnabled(),o=this.hasTelegramConfig();return`
...
`)}</textarea>
    ...
    `}hasTelegramConfig(){const e=window.NEXT_PUBLIC_TELEGRAM_BOT_TOKEN||U.NEXT_PUBLIC_TELEGRAM_BOT_TOKEN,t=window.NEXT_PUBLIC_TELEGRAM_CHAT_ID||U.NEXT_PUBLIC_TELEGRAM_CHAT_ID;return!!(e&&t)}getTelegramEnabled(){return localStorage.getItem("telegram_enabled")==="true"}setTelegramEnabled(e){localStorage.setItem("telegram_enabled",e.toString())}}class X{constructor(){l(this,"plugins",new Map)}register(e){this.plugins.has(e.id)&&console.warn(`Plugin ${e.id} already registered, replacing...`),this.plugins.set(e.id,e),console.log(`✅ Plugin registered: ${e.name} (${e.id})`)}unregister(e){const t=this.plugins.get(e);t&&(t.dispose(),this.plugins.delete(e),console.log(`❌ Plugin unregistered: ${t.name} (${e})`))}getPlugin(e){return this.plugins.get(e)}getAllPlugins(){return Array.from(this.plugins.values())}getEnabledPlugins(){return this.getAllPlugins().filter(e=>e.enabled&&e.isAvailable()).sort((e,t)=>t.priority-e.priority)}async initializeAll(){const e=this.getAllPlugins();console.log(`🔌 Initializing ${e.length} plugins...`),await Promise.all(e.map(async t=>{try{await t.initialize(),console.log(`✅ Plugin initialized: ${t.name}`)}catch(n){console.error(`❌ Failed to initialize plugin ${t.name}:`,n)}}))}async search(e,t=5){const n=this.getEnabledPlugins();if(n.length===0)return console.warn("No enabled plugins available for search"),[];console.log(`🔍 Searching with ${n.length} plugins:`,n.map(r=>r.name));const i=(await Promise.all(n.map(async r=>{try{return(await r.search(e,t)).map(a=>({...a,metadata:{...a.metadata,pluginId:r.id,pluginName:r.name,priority:r.priority}}))}catch(s){return console.error(`Error searching with plugin ${r.name}:`,s),[]}}))).flat();return i.sort((r,s)=>{var c,h;const a=((c=r.metadata)==null?void 0:c.priority)||0,d=((h=s.metadata)==null?void 0:h.priority)||0;return a!==d?d-a:(s.score||0)-(r.score||0)}),i.slice(0,t)}disposeAll(){this.plugins.forEach(e=>e.dispose()),this.plugins.clear(),console.log("🧹 All plugins disposed")}}class Z{constructor(){l(this,"id","manual-index");l(this,"name","手動索引");l(this,"description","搜尋管理員手動新增的索引內容");l(this,"priority",10);l(this,"enabled",!0)}async initialize(){const e=C.getAll();console.log(`📚 Manual Index Plugin: ${e.length} indexes loaded`)}async search(e,t=5){try{return C.search(e,t).map(({index:o,score:i})=>({type:"manual-index",title:o.name,snippet:o.content.substring(0,200),content:o.content,url:`#manual-index-${o.id}`,score:i,metadata:{description:o.description,createdAt:o.createdAt,indexId:o.id}}))}catch(n){return console.error("Error in ManualIndexPlugin.search:",n),[]}}isAvailable(){return C.getAll().length>0}getConfig(){return{enabled:this.enabled,priority:this.priority,indexCount:C.getAll().length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class ee{constructor(){l(this,"id","frontend-pages");l(this,"name","前端頁面");l(this,"description","搜尋當前網站已索引的頁面內容");l(this,"priority",8);l(this,"enabled",!0);l(this,"extractor");this.extractor=new T}async initialize(){const e=m.loadIndexedPages();console.log(`📄 Frontend Page Plugin: ${e.length} pages loaded`)}async search(e,t=5){try{const n=m.loadIndexedPages();if(n.length===0)return[];const o=this.extractor.extractKeywords(e);return n.map(r=>{const s=`${r.title} ${r.snippet}`.toLowerCase(),d=o.filter(c=>s.includes(c.toLowerCase())).length/o.length;return{page:r,score:d}}).filter(r=>r.score>0).sort((r,s)=>s.score-r.score).slice(0,t).map(({page:r,score:s})=>({type:"frontend-page",title:r.title,snippet:r.snippet,content:r.snippet,url:r.url,score:s,metadata:{keywords:r.keywords,pageId:r.id}}))}catch(n){return console.error("Error in FrontendPagePlugin.search:",n),[]}}isAvailable(){return m.loadIndexedPages().length>0}getConfig(){return{enabled:this.enabled,priority:this.priority,pageCount:m.loadIndexedPages().length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class z{static getAll(){const e=localStorage.getItem(this.STORAGE_KEY);if(!e)return[];try{return JSON.parse(e)}catch(t){return console.error("Failed to parse sitemap configs:",t),[]}}static getById(e){return this.getAll().find(n=>n.id===e)||null}static async create(e){const t={id:this.generateId(),domain:e.domain,sitemapUrl:e.sitemapUrl,enabled:!0,autoUpdate:e.autoUpdate||!1,updateInterval:e.updateInterval||60,lastUpdated:0,pages:[]},n=this.getAll();return n.push(t),this.saveAll(n),console.log("Created sitemap config:",t.id),await this.crawl(t.id),t.autoUpdate&&this.startAutoUpdate(t.id),t}static update(e,t){const n=this.getAll(),o=n.find(i=>i.id===e);return o?(t.domain!==void 0&&(o.domain=t.domain),t.sitemapUrl!==void 0&&(o.sitemapUrl=t.sitemapUrl),t.enabled!==void 0&&(o.enabled=t.enabled),t.autoUpdate!==void 0&&(o.autoUpdate=t.autoUpdate),t.updateInterval!==void 0&&(o.updateInterval=t.updateInterval),this.saveAll(n),o.autoUpdate?this.startAutoUpdate(e):this.stopAutoUpdate(e),console.log("Updated sitemap config:",e),o):null}static delete(e){const t=this.getAll(),n=t.filter(o=>o.id!==e);return n.length===t.length?!1:(this.saveAll(n),this.stopAutoUpdate(e),console.log("Deleted sitemap config:",e),!0)}static async crawl(e){const t=this.getById(e);if(!t)throw new Error("Sitemap config not found");console.log("Crawling sitemap:",t.sitemapUrl);try{const o=await(await fetch(t.sitemapUrl)).text(),r=new DOMParser().parseFromString(o,"text/xml"),s=Array.from(r.querySelectorAll("url loc")).map(S=>S.textContent||"");console.log(`Found ${s.length} URLs in sitemap`);const d=s.slice(0,50),c=[];for(const S of d)try{const p=await this.crawlPage(S);p&&c.push(p)}catch(p){console.error(`Failed to crawl ${S}:`,p)}t.pages=c,t.lastUpdated=Date.now();const h=this.getAll(),w=h.findIndex(S=>S.id===e);w>=0&&(h[w]=t,this.saveAll(h)),console.log(`Crawled ${c.length} pages successfully`)}catch(n){throw console.error("Failed to crawl sitemap:",n),n}}static async crawlPage(e){var t;try{const o=await(await fetch(e)).text(),r=new DOMParser().parseFromString(o,"text/html"),s=((t=r.querySelector("title"))==null?void 0:t.textContent)||e,a=new T,d=a.extractText(r.body),c=a.extractKeywords(d),h=a.generateFingerprint(d);return{url:e,title:s,content:d.substring(0,5e3),keywords:c,fingerprint:h,lastCrawled:Date.now()}}catch(n){return console.error(`Failed to crawl page ${e}:`,n),null}}static search(e,t,n=5){const o=this.getAll().filter(c=>c.enabled),i=t&&t.length>0?o.filter(c=>t.includes(c.domain)):o;if(i.length===0)return[];const r=new T,s=r.extractKeywords(e),a=r.generateFingerprint(e),d=[];for(const c of i)for(const h of c.pages){const w=this.calculateSimilarity(s,a,h.keywords,h.fingerprint);w>0&&d.push({page:h,domain:c.domain,score:w})}return d.sort((c,h)=>h.score-c.score).slice(0,n)}static calculateSimilarity(e,t,n,o){const i=this.calculateKeywordScore(e,n),r=this.calculateFingerprintScore(t,o);return i*.5+r*.5}static calculateKeywordScore(e,t){return e.length===0||t.length===0?0:e.filter(o=>t.includes(o)).length/Math.max(e.length,t.length)}static calculateFingerprintScore(e,t){if(e.length===0||t.length===0)return 0;let n=0,o=0;for(let i=0;i<Math.max(e.length,t.length);i++){const r=e[i]||0,s=t[i]||0;r===1&&s===1&&n++,(r===1||s===1)&&o++}return o>0?n/o:0}static startAutoUpdate(e){this.stopAutoUpdate(e);const t=this.getById(e);if(!t||!t.autoUpdate)return;const n=t.updateInterval*60*1e3,o=window.setInterval(()=>{console.log(`Auto-updating sitemap: ${e}`),this.crawl(e).catch(i=>console.error("Auto-update failed:",i))},n);this.updateTimers.set(e,o)}static stopAutoUpdate(e){const t=this.updateTimers.get(e);t&&(clearInterval(t),this.updateTimers.delete(e))}static initAutoUpdates(){const e=this.getAll().filter(t=>t.enabled&&t.autoUpdate);for(const t of e)this.startAutoUpdate(t.id);console.log(`Initialized ${e.length} auto-update timers`)}static saveAll(e){localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}static generateId(){return"sitemap_"+Date.now().toString(36)+Math.random().toString(36).substring(2)}static clearAll(){this.updateTimers.forEach(e=>clearInterval(e)),this.updateTimers.clear(),localStorage.removeItem(this.STORAGE_KEY)}}l(z,"STORAGE_KEY","sm_sitemap_configs"),l(z,"updateTimers",new Map);class te{constructor(){l(this,"id","sitemap");l(this,"name","Sitemap 索引");l(this,"description","搜尋外部網站的 Sitemap 內容");l(this,"priority",6);l(this,"enabled",!1);l(this,"extractor");this.extractor=new T}async initialize(){const e=z.getAll();console.log(`🗺️ Sitemap Plugin: ${e.length} sitemaps loaded`),e.length>0&&(this.enabled=!0)}async search(e,t=5){try{const n=z.getAll();if(n.length===0)return[];const o=[],i=this.extractor.extractKeywords(e);for(const r of n)try{const s=await z.search(r.id,i,3);o.push(...s.map(({page:a,score:d})=>({type:"sitemap",title:a.title,snippet:a.content.substring(0,200),content:a.content.substring(0,500),url:a.url,score:d,metadata:{domain:r.domain,lastUpdated:r.lastUpdated,sitemapId:r.id}})))}catch(s){console.error(`Error searching sitemap ${r.domain}:`,s)}return o.sort((r,s)=>(s.score||0)-(r.score||0)).slice(0,t)}catch(n){return console.error("Error in SitemapPlugin.search:",n),[]}}isAvailable(){return z.getAll().length>0}getConfig(){return{enabled:this.enabled,priority:this.priority,sitemapCount:z.getAll().length}}updateConfig(e){typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority)}dispose(){}}class ne{constructor(e){l(this,"id","sql-database");l(this,"name","SQL 資料庫");l(this,"description","搜尋 SQL 資料庫中的內容");l(this,"priority",5);l(this,"enabled",!1);l(this,"config");l(this,"extractor");this.config={enabled:!1,priority:5,searchTable:"knowledge_base",searchColumns:["title","content"],titleColumn:"title",contentColumn:"content",urlColumn:"url",...e},this.enabled=this.config.enabled,this.priority=this.config.priority,this.extractor=new T}async initialize(){if(!this.config.connectionId){console.warn("⚠️ SQL Plugin: No connection ID configured"),this.enabled=!1;return}if(!this.config.apiEndpoint){console.warn("⚠️ SQL Plugin: No API endpoint configured"),this.enabled=!1;return}try{if(!k.getById(this.config.connectionId)){console.warn(`⚠️ SQL Plugin: Connection ${this.config.connectionId} not found`),this.enabled=!1;return}if(!await k.testConnection(this.config.connectionId,this.config.apiEndpoint)){console.warn("⚠️ SQL Plugin: Connection test failed"),this.enabled=!1;return}console.log("✅ SQL Plugin: Connection test successful")}catch(e){console.error("❌ SQL Plugin initialization error:",e),this.enabled=!1}}async search(e,t=5){if(!this.isAvailable())return[];try{const n=this.extractor.extractKeywords(e,5),o=this.buildSearchQuery(n,t),i=await k.query(this.config.connectionId,o,this.config.apiEndpoint);return this.convertToSources(i)}catch(n){return console.error("Error in SQLPlugin.search:",n),[]}}buildSearchQuery(e,t){const{searchTable:n,searchColumns:o,titleColumn:i,contentColumn:r,urlColumn:s}=this.config,a=o.map(d=>e.map(c=>`${d} LIKE '%${c}%'`).join(" OR ")).join(" OR ");return`
      SELECT 
        ${i} as title,
        ${r} as content,
        ${s} as url
      ...
      WHERE ${a}
      LIMIT ${t}
    `.trim()}convertToSources(e){return e.map((t,n)=>({type:"sql",title:t.title||`結果 ${n+1}`,snippet:t.content?t.content.substring(0,200):"",content:t.content||"",url:t.url||"#",score:1-n*.1,metadata:{source:"sql-database",connectionId:this.config.connectionId,table:this.config.searchTable}}))}isAvailable(){return this.enabled&&!!this.config.connectionId&&!!this.config.apiEndpoint&&!!this.config.searchTable}getConfig(){return{...this.config}}updateConfig(e){this.config={...this.config,...e},typeof e.enabled=="boolean"&&(this.enabled=e.enabled),typeof e.priority=="number"&&(this.priority=e.priority),this.initialize().catch(t=>{console.error("Error reinitializing SQL Plugin:",t)})}dispose(){this.enabled=!1}}function oe(){const x=localStorage.getItem("sm_sql_plugin_config"),e=x?JSON.parse(x):{};return new ne(e)}function ie(){const x=new X;return x.register(new Z),x.register(new ee),x.register(new te),x.register(oe()),x}function re(x){const e=localStorage.getItem("sm_plugin_configs");if(e)try{const t=JSON.parse(e);Object.keys(t).forEach(n=>{const o=x.getPlugin(n);o&&o.updateConfig(t[n])}),console.log("✅ Plugin configs loaded from localStorage")}catch(t){console.error("Error loading plugin configs:",t)}}var F={};class se{constructor(){l(this,"config");l(this,"openAI");l(this,"indexing");l(this,"agent");l(this,"panel");l(this,"capture");l(this,"conversationState");l(this,"initialized",!1);l(this,"captureMode",!1);l(this,"adminPanel");l(this,"pluginManager")}init(e){var r,s,a;if(this.initialized){console.warn("ServiceModuler already initialized");return}this.config=e,$.getCurrentUser(),console.log("User ID:",$.getUserId()),this.pluginManager=ie(),re(this.pluginManager),this.pluginManager.initializeAll().then(()=>{console.log("✅ All plugins initialized")}).catch(d=>{console.error("❌ Plugin initialization error:",d)}),this.openAI=new I(e.azureOpenAI||e.llmAPI),this.indexing=new P(this.openAI,e.siteConfig);const t=localStorage.getItem("telegram_enabled")==="true",n=window.NEXT_PUBLIC_TELEGRAM_BOT_TOKEN||F.NEXT_PUBLIC_TELEGRAM_BOT_TOKEN,o=window.NEXT_PUBLIC_TELEGRAM_CHAT_ID||F.NEXT_PUBLIC_TELEGRAM_CHAT_ID,i=t&&n&&o?{botToken:n,chatId:o}:void 0;this.agent=new j(this.openAI,this.pluginManager,e.rules||[],i),this.capture=new G,this.panel=new V(((r=e.ui)==null?void 0:r.width)||"33.33%",((s=e.ui)==null?void 0:s.position)||"right"),this.panel.setCallbacks({onSendMessage:(d,c)=>this.handleSendMessage(d,c),onSelectRule:d=>this.handleSelectRule(d),onClose:()=>this.handleClose(),onOpen:()=>this.handleOpen()}),this.loadConversationState(),this.agent&&this.panel.setRules(this.agent.getRules(),(a=this.agent.getCurrentRule())==null?void 0:a.id),this.adminPanel=new W,this.initialized=!0,e.debug&&console.log("ServiceModuler initialized",e)}open(){var e;if(!this.initialized){console.error("ServiceModuler not initialized. Call init() first.");return}(e=this.panel)==null||e.open()}close(){var e;(e=this.panel)==null||e.close()}async sendMessage(e,t){var o,i,r,s,a;if(!this.initialized||!this.agent||!this.panel||!this.openAI){console.error("ServiceModuler not initialized");return}const n={role:"user",content:e||"請分析這張圖片",timestamp:Date.now()};(o=this.conversationState)==null||o.messages.push(n),this.panel.addMessage(n),this.saveConversationState();try{let d,c,h=!1;const w=((i=this.conversationState)==null?void 0:i.sessionId)||this.generateSessionId(),S=localStorage.getItem("lens_service_user_id")||"default_user";if(t)d=await this.openAI.chatCompletionWithImage(e||"請分析這張圖片並回答問題",t,((r=this.conversationState)==null?void 0:r.messages.slice(0,-1))||[]);else{const g=await this.agent.processMessage(e,((s=this.conversationState)==null?void 0:s.messages)||[],w,S);d=g.response,c=g.sources,h=g.needsHumanReply}const p={role:"assistant",content:d,timestamp:Date.now(),sources:c};(a=this.conversationState)==null||a.messages.push(p),this.panel.addMessage(p),this.saveConversationState(),await this.saveConversationToDatabase(w,S)}catch(d){console.error("Error processing message:",d);const c={role:"assistant",content:`抱歉，發生錯誤：${d instanceof Error?d.message:"未知錯誤"}`,timestamp:Date.now()};this.panel.addMessage(c)}}async saveConversationToDatabase(e,t){if(this.conversationState)try{if(!(await fetch("/api/conversations",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:t,conversationId:e,messages:this.conversationState.messages})})).ok)throw new Error("Failed to save conversation");console.log("✅ Conversation saved to database")}catch(n){console.error("Failed to save conversation to database:",n)}}setRule(e){var t;this.agent&&(this.agent.setRule(e),this.panel&&this.panel.setRules(this.agent.getRules(),(t=this.agent.getCurrentRule())==null?void 0:t.id))}async indexSite(e,t="domain",n){if(!this.indexing){console.error("Indexing service not initialized");return}const o=e||window.location.origin;await this.indexing.indexSite(o,t,n)}enableCaptureMode(){if(!this.capture||!this.panel){console.error("Capture service not initialized");return}this.captureMode=!0,this.capture.enable((e,t,n)=>{console.log("Element captured:",{text:t,element:n}),this.open(),this.panel.setCapturedImage(e,t)}),console.log("Capture mode enabled. Press Ctrl+Click to capture elements.")}disableCaptureMode(){this.capture&&(this.capture.disable(),this.captureMode=!1)}searchCurrentPage(e){return D.searchInCurrentPage(e).map(n=>({text:n.text,context:n.context}))}getCurrentPageContent(){return D.extractCurrentPageContent()}clearConversation(){var e;this.conversationState&&(this.conversationState.messages=[],this.saveConversationState()),(e=this.panel)==null||e.clearMessages()}openAdmin(){if(!this.initialized){console.error("ServiceModuler not initialized. Call init() first.");return}if(!this.adminPanel){console.error("AdminPanel not initialized");return}this.adminPanel.open()}destroy(){var e,t;(e=this.panel)==null||e.destroy(),(t=this.adminPanel)==null||t.close(),this.initialized=!1}handleSendMessage(e,t){this.sendMessage(e,t)}handleSelectRule(e){this.setRule(e)}handleOpen(){console.log("✅ Panel opened")}handleClose(){this.saveConversationState(),console.log("❌ Panel closed")}loadConversationState(){let e=m.loadConversation();e||(e={sessionId:this.generateSessionId(),messages:[]}),this.conversationState=e,this.panel&&e.messages.length>0&&e.messages.forEach(t=>{this.panel.addMessage(t)})}saveConversationState(){this.conversationState&&m.saveConversation(this.conversationState)}generateSessionId(){return`sm_${Date.now()}_${Math.random().toString(36).substr(2,9)}`}}const K=new se;return typeof window<"u"&&(window.LensService=K),K});
