(function(f,w){typeof exports=="object"&&typeof module<"u"?module.exports=w():typeof define=="function"&&define.amd?define(w):(f=typeof globalThis<"u"?globalThis:f||self,f.LensService=w())})(this,function(){"use strict";var Q=Object.defineProperty;var V=(f,w,A)=>w in f?Q(f,w,{enumerable:!0,configurable:!0,writable:!0,value:A}):f[w]=A;var m=(f,w,A)=>V(f,typeof w!="symbol"?w+"":w,A);const f={container:`
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
  ...
  `,tabButtonActive:`
    ...
    color: white;
    border-color: #6366f1;
  `};class w{constructor(e="33.33%",t="right"){m(this,"container");m(this,"overlay");m(this,"panel");m(this,"isOpen",!1);m(this,"width");m(this,"position");m(this,"capturedImage",null);m(this,"capturedText",null);m(this,"onSendMessage");m(this,"onSelectRule");m(this,"onClose");m(this,"onOpen");this.width=e,this.position=t,this.container=this.createContainer(),this.overlay=this.createOverlay(),this.panel=this.createPanel()}createContainer(){const e=document.createElement("div");return e.id="sm-container",e.style.cssText=f.container,e}createOverlay(){const e=document.createElement("div");return e.style.cssText=f.overlay,e.style.display="none",e.addEventListener("click",()=>this.close()),e}createPanel(){const e=document.createElement("div");return e.style.cssText=f.panel,e.style.width=this.width,this.position==="right"?(e.style.right=`-${this.width}`,e.style.left="auto"):(e.style.left=`-${this.width}`,e.style.right="auto"),e.innerHTML=`
    ...
    `,this.bindEvents(e),e}bindEvents(e){var n,s,i,r,l;(n=e.querySelector("#sm-close-btn"))==null||n.addEventListener("click",()=>{this.close()});const t=e.querySelector("#sm-send-btn");t?(console.log("✅ Send button found, binding click event"),t.addEventListener("click",a=>{console.log("🔥 Send button clicked via addEventListener!"),a.preventDefault(),a.stopPropagation(),this.handleSend()}),t.onclick=a=>{console.log("🔥 Send button clicked via onclick!"),a.preventDefault(),a.stopPropagation(),this.handleSend()},e.addEventListener("click",a=>{(a.target.id==="sm-send-btn"||a.target.closest("#sm-send-btn"))&&(console.log("🔥 Send button clicked via delegation!"),a.preventDefault(),a.stopPropagation(),this.handleSend())})):console.error("❌ Send button not found!");const o=e.querySelector("#sm-input");o?(console.log("✅ Input field found, binding events"),o.addEventListener("keypress",a=>{a.key==="Enter"&&(console.log("🔥 Enter key pressed in input"),this.handleSend())}),o.addEventListener("input",a=>{console.log("🔥 Input event:",a.target.value)}),o.addEventListener("focus",()=>{console.log("🔥 Input focused")}),o.addEventListener("blur",()=>{console.log("🔥 Input blurred")})):console.error("❌ Input field not found!"),(s=e.querySelector("#sm-chat-tab"))==null||s.addEventListener("click",()=>{this.showView("chat")}),(i=e.querySelector("#sm-refresh-btn"))==null||i.addEventListener("click",()=>{this.clearMessages()}),(r=e.querySelector("#sm-history-btn"))==null||r.addEventListener("click",()=>{this.showHistory()}),(l=e.querySelector("#sm-remove-image"))==null||l.addEventListener("click",()=>{this.clearCapturedImage()})}handleSend(){const e=this.panel.querySelector("#sm-input"),t=e.value.trim();(t||this.capturedImage)&&this.onSendMessage&&(this.onSendMessage(t,this.capturedImage||void 0,this.capturedText||void 0),e.value="",this.clearCapturedImage())}showView(e){const t=this.panel.querySelector("#sm-chat-view"),o=this.panel.querySelector("#sm-chat-tab");e==="chat"&&(t.style.display="flex",o.style.cssText=f.tabButton+"; "+f.tabButtonActive)}addMessage(e){const t=this.panel.querySelector("#sm-messages");if(!t)return;const o=document.createElement("div");if(o.style.cssText=e.role==="user"?f.userMessage:f.assistantMessage,e.role==="assistant"?o.innerHTML=e.content:o.textContent=e.content,e.sources&&e.sources.length>0){const n=document.createElement("div");n.style.cssText=f.sources,n.innerHTML="<strong>參考來源：</strong><br>",e.sources.forEach((s,i)=>{const r=document.createElement("a");r.href=s.url,r.target="_blank",r.textContent=`[${i+1}] ${s.title}`,r.style.cssText=f.sourceLink,n.appendChild(r),n.appendChild(document.createElement("br"))}),o.appendChild(n)}t.appendChild(o),setTimeout(()=>{t.scrollTop=t.scrollHeight},10)}setRules(e,t){}clearMessages(){const e=this.panel.querySelector("#sm-messages");e&&(e.innerHTML="")}async showHistory(){try{const{DatabaseService:e}=await Promise.resolve().then(()=>v),t=await e.getConversations();this.showHistoryView(t)}catch(e){console.error("Failed to load history:",e),alert("載入歷史記錄失敗")}}showHistoryView(e){const t=this.panel.querySelector("#sm-chat-view");if(console.log("📋 showHistoryView called with",e.length,"conversations"),console.log("📋 chatView:",t),!t){console.error("❌ chatView not found");return}t.style.display="none";let o=this.panel.querySelector("#sm-history-view");if(!o){o=document.createElement("div"),o.id="sm-history-view",o.style.cssText=f.chatView;const i=t.parentElement;if(console.log("📋 parent element:",i),i)i.appendChild(o),console.log("✅ History view created and appended");else{console.error("❌ Parent element not found");return}}if(o.style.display="flex",o.style.flexDirection="column",console.log("✅ History view display set to flex"),!Array.isArray(e)||e.length===0)o.innerHTML=`
      ...
      `;else{const i=e.map(r=>{let l=[];try{l=typeof r.messages=="string"?JSON.parse(r.messages):r.messages}catch{l=[]}const a=Array.isArray(l)?l.length:0,c=new Date(r.created_at).toLocaleString("zh-TW",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}),d=r.conversation_id||r.id||"unknown",p=d.toString().slice(-8);return`
    ...
    `}showEditDialog(e,t,o=!1){return new Promise(n=>{const s=document.createElement("div");s.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
        ...
        align-items: center;
        z-index: 1000000;
      `;const i=o?`<textarea id="edit-input" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; min-height: 120px; resize: vertical; font-family: inherit; color: #1f2937; background: #ffffff;">${t}</textarea>`:`<input type="text" id="edit-input" value="${t}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; color: #1f2937; background: #ffffff;">`;s.innerHTML=`
      ...
      `,document.body.appendChild(s);const r=s.querySelector("#edit-input"),l=s.querySelector("#cancel-btn"),a=s.querySelector("#save-btn");r.focus(),r instanceof HTMLInputElement?r.select():r.setSelectionRange(0,r.value.length),l==null||l.addEventListener("click",()=>{document.body.removeChild(s),n(null)}),a==null||a.addEventListener("click",()=>{const c=r.value.trim();document.body.removeChild(s),n(c)}),r instanceof HTMLInputElement&&r.addEventListener("keydown",c=>{if(c.key==="Enter"){const d=r.value.trim();document.body.removeChild(s),n(d)}}),s.addEventListener("click",c=>{c.target===s&&(document.body.removeChild(s),n(null))})})}showConfirmDialog(e){return new Promise(t=>{var i,r;const o=document.createElement("div");o.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      `,o.appendChild(n),document.body.appendChild(o);const s=l=>{document.body.removeChild(o),t(l)};(i=n.querySelector("#confirm-ok"))==null||i.addEventListener("click",()=>s(!0)),(r=n.querySelector("#confirm-cancel"))==null||r.addEventListener("click",()=>s(!1)),o.addEventListener("click",l=>{l.target===o&&s(!1)})})}showAlertDialog(e){return new Promise(t=>{var i;const o=document.createElement("div");o.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      `,o.appendChild(n),document.body.appendChild(o);const s=()=>{document.body.removeChild(o),t()};(i=n.querySelector("#alert-ok"))==null||i.addEventListener("click",s),o.addEventListener("click",r=>{r.target===o&&s()})})}updateNavHighlight(){if(!this.container)return;this.container.querySelectorAll(".nav-item").forEach(t=>{const o=t,n=o.dataset.page===this.currentPage;o.style.background=n?"#ede9fe":"transparent",o.style.color=n?"#7c3aed":"#4b5563",o.style.fontWeight=n?"600":"500",n?o.classList.add("active"):o.classList.remove("active")})}bindEvents(){if(!this.container)return;const e=this.container.querySelector("#admin-login-form");if(e){e.addEventListener("submit",async p=>{p.preventDefault(),p.stopPropagation();const h=this.container.querySelector("#admin-username"),u=this.container.querySelector("#admin-password"),g=(h==null?void 0:h.value)||"",b=(u==null?void 0:u.value)||"";console.log("Login attempt with username:",g);try{const{DatabaseService:y}=await Promise.resolve().then(()=>v),S=await y.validateAdmin(g,b);console.log("Login successful (database auth)"),this.isAuthenticated=!0,this.container.innerHTML=this.renderAdminUI(),await this.updatePageContent(),this.bindEvents()}catch(y){console.error("Login error:",y),this.showAlertDialog("登入時發生錯誤，請稍後再試").then(()=>{u.value="",u.focus()})}});const d=this.container.querySelector("#admin-username");d&&setTimeout(()=>{d.focus()},100)}setTimeout(()=>{const d=this.container.querySelectorAll(".nav-item");if(console.log("Binding nav items, found:",d.length),d.length===0&&this.isAuthenticated){console.warn("Nav items not found, retrying..."),setTimeout(()=>this.bindEvents(),100);return}d.forEach((p,h)=>{console.log(`Binding nav item ${h}:`,p.dataset.page);const u=p.cloneNode(!0);p.parentNode.replaceChild(u,p),u.addEventListener("click",async()=>{const g=u.dataset.page;console.log("Nav item clicked:",g),g&&g!==this.currentPage&&(this.currentPage=g,await this.updatePageContent(),this.updateNavHighlight())})})},50);const t=this.container.querySelector("#admin-logout");t&&t.addEventListener("click",()=>{this.isAuthenticated=!1,this.container.innerHTML=this.renderLoginUI(),this.bindEvents()});const o=this.container.querySelector("#telegram-settings-form");o&&o.addEventListener("submit",async d=>{d.preventDefault(),d.stopPropagation();const p=this.container.querySelector("#telegram-enabled"),h=(p==null?void 0:p.checked)||!1;this.setTelegramEnabled(h),alert(`Telegram 通知已${h?"啟用":"停用"}`),await this.updatePageContent()});const n=this.container.querySelector("#change-password-form");n&&n.addEventListener("submit",async d=>{d.preventDefault(),d.stopPropagation();const p=this.container.querySelector("#new-password");if(((p==null?void 0:p.value)||"").length<4){alert("密碼長度至少 4 個字元");return}alert("密碼已更新"),await this.updatePageContent()});const s=this.container.querySelector("#ip-whitelist-form");s&&s.addEventListener("submit",async d=>{d.preventDefault(),d.stopPropagation();const p=this.container.querySelector("#ip-list"),u=((p==null?void 0:p.value)||"").split(`
`).map(g=>g.trim()).filter(g=>g.length>0);this.saveIPWhitelist(u),alert(`已更新 IP 白名單（${u.length} 個 IP）`),await this.updatePageContent()});const i=this.container.querySelector("#api-config-form");i&&i.addEventListener("submit",d=>{var p,h,u,g,b,y;d.preventDefault(),d.stopPropagation(),(p=this.container.querySelector("#llm-endpoint"))!=null&&p.value,(h=this.container.querySelector("#llm-api-key"))!=null&&h.value,(u=this.container.querySelector("#llm-deployment"))!=null&&u.value,(g=this.container.querySelector("#embed-endpoint"))!=null&&g.value,(b=this.container.querySelector("#embed-api-key"))!=null&&b.value,(y=this.container.querySelector("#embed-deployment"))!=null&&y.value,alert("API 設定已儲存")});const r=this.container.querySelector("#agent-tool-config-form");r&&r.addEventListener("submit",async d=>{var p,h;d.preventDefault(),d.stopPropagation(),(p=this.container.querySelector("#manual-index-enabled"))!=null&&p.checked,(h=this.container.querySelector("#frontend-pages-enabled"))!=null&&h.checked,alert("Agent 設定已儲存"),await this.updatePageContent()});const l=this.container.querySelector("#sql-plugin-config-form");l&&l.addEventListener("submit",async d=>{var x,M,q,D,T,_,O,j;d.preventDefault(),d.stopPropagation();const p=((x=this.container.querySelector("#sql-plugin-enabled"))==null?void 0:x.checked)||!1,h=parseInt(((M=this.container.querySelector("#sql-plugin-priority"))==null?void 0:M.value)||"5"),u=((q=this.container.querySelector("#sql-api-endpoint"))==null?void 0:q.value)||"",g=((D=this.container.querySelector("#sql-connection-id"))==null?void 0:D.value)||"",b=((T=this.container.querySelector("#sql-search-table"))==null?void 0:T.value)||"knowledge_base",y=((_=this.container.querySelector("#sql-title-column"))==null?void 0:_.value)||"title",S=((O=this.container.querySelector("#sql-content-column"))==null?void 0:O.value)||"content",z=((j=this.container.querySelector("#sql-url-column"))==null?void 0:j.value)||"url",P={enabled:p,priority:h,apiEndpoint:u,connectionId:g,searchTable:b,titleColumn:y,contentColumn:S,urlColumn:z};localStorage.setItem("sm_sql_plugin_config",JSON.stringify(P)),alert("SQL Plugin 設定已儲存"),await this.updatePageContent()});const a=this.container.querySelector("#sql-connection-form");a&&a.addEventListener("submit",async d=>{var h,u;d.preventDefault(),d.stopPropagation();const p=((h=this.container.querySelector("#sql-conn-name"))==null?void 0:h.value)||"";if((u=this.container.querySelector("#sql-conn-type"))==null||u.value,!p){alert("請輸入連接名稱");return}try{alert("SQL 連接已新增"),await this.updatePageContent()}catch(g){console.error("Error creating SQL connection:",g),alert("新增失敗")}}),this.container.querySelectorAll(".delete-sql-connection").forEach(d=>{d.addEventListener("click",async()=>{if(d.dataset.id&&confirm("確定要刪除這個連接嗎？"))try{alert("連接已刪除"),await this.updatePageContent()}catch(h){console.error("Error deleting SQL connection:",h),alert("刪除失敗")}})})}renderAdminUI(){return`
    ...
    `}async renderPageContent(){switch(this.currentPage){case"dashboard":return await this.renderDashboard();case"manual-index":return await this.renderManualIndex();case"conversations":return await this.renderConversations();case"system":return await this.renderSystemSettings();default:return"<p>頁面不存在</p>"}}async updatePageContent(){const e=this.container.querySelector("#admin-content");e&&(e.innerHTML=await this.renderPageContent(),this.bindContentEvents())}bindContentEvents(){this.container&&(this.bindManualIndexEvents(),this.bindCustomerServiceEvents(),this.bindAdminUserEvents(),this.bindSystemSettingsEvents())}bindManualIndexEvents(){const e=this.container.querySelector("#add-index-btn");e&&e.addEventListener("click",async()=>{await this.showAddIndexModal()});const t=this.container.querySelector("#generate-embeddings-btn");t&&t.addEventListener("click",async()=>{try{const s=t;s.disabled=!0,s.textContent="生成中...";const r=(await C.getAll()).length;await this.showAlertDialog(`成功為 ${r} 個索引生成了向量嵌入`),await this.updatePageContent()}catch(s){await this.showAlertDialog(`生成失敗：${s instanceof Error?s.message:"未知錯誤"}`)}finally{const s=t;s.disabled=!1,s.textContent="生成所有Embeddings"}}),this.container.querySelectorAll(".edit-index-btn").forEach(s=>{s.addEventListener("click",async()=>{const i=s.dataset.id;i&&await this.showEditIndexModal(i)})}),this.container.querySelectorAll(".delete-index-btn").forEach(s=>{s.addEventListener("click",async()=>{const i=s.dataset.id;i&&await this.showDeleteConfirmDialog(i)})})}bindCustomerServiceEvents(){const e=this.container.querySelector("#refresh-conversations");e&&e.addEventListener("click",async()=>{await this.updatePageContent()}),this.container.querySelectorAll(".view-conversation-btn").forEach(n=>{n.addEventListener("click",async s=>{const i=s.target.getAttribute("data-id");i&&await this.showConversationModal(i)})}),this.container.querySelectorAll(".delete-conversation-btn").forEach(n=>{n.addEventListener("click",async s=>{const i=s.target.getAttribute("data-id");if(i&&await this.showConfirmDialog("確定要刪除這個對話嗎？此操作無法復原。"))try{const{CustomerServiceManager:l}=await Promise.resolve().then(()=>L);await l.deleteConversation(i),await this.showAlertDialog("對話已刪除"),await this.updatePageContent()}catch(l){await this.showAlertDialog(`刪除失敗：${l instanceof Error?l.message:"未知錯誤"}`)}})})}bindAdminUserEvents(){}bindSystemSettingsEvents(){const e=this.container.querySelector("#edit-default-reply-btn");e&&e.addEventListener("click",async()=>{const i=this.container.querySelector("#default-reply-display"),r=i.textContent||"",l=await this.showEditDialog("編輯預設回覆",r,!0);if(l!==null)try{const{DatabaseService:a}=await Promise.resolve().then(()=>v);await a.setSetting("default_reply",l),i.textContent=l,await this.showAlertDialog("預設回覆已更新")}catch(a){console.error("Failed to save default reply:",a),await this.showAlertDialog("儲存失敗，請稍後再試")}});const t=this.container.querySelector("#edit-system-prompt-btn");t&&t.addEventListener("click",async()=>{const i=this.container.querySelector("#system-prompt-display"),r=i.textContent||"",l=await this.showEditDialog("編輯系統提示詞",r,!0);if(l!==null)try{const{DatabaseService:a}=await Promise.resolve().then(()=>v);await a.setSetting("system_prompt",l),i.textContent=l,await this.showAlertDialog("系統提示詞已更新")}catch(a){console.error("Failed to save system prompt:",a),await this.showAlertDialog("儲存失敗，請稍後再試")}});const o=this.container.querySelector("#edit-llms-txt-url-btn");o&&o.addEventListener("click",async()=>{const i=this.container.querySelector("#llms-txt-url-display");if(i){const r=i.textContent==="未設定"?"":i.textContent||"",l=await this.showEditDialog("編輯 LLMs.txt 網址",r,!1);if(l!==null)try{const{DatabaseService:a}=await Promise.resolve().then(()=>v);await a.setSetting("llms_txt_url",l),i.textContent=l||"未設定",await this.showAlertDialog("LLMs.txt 網址已更新")}catch(a){console.error("Failed to save llms txt url:",a),await this.showAlertDialog("儲存失敗，請稍後再試")}}});const n=this.container.querySelector("#add-admin-user-btn");n&&n.addEventListener("click",async()=>{await this.showAddAdminUserModal()}),this.container.querySelectorAll(".delete-admin-user-btn").forEach(i=>{i.addEventListener("click",async()=>{const r=i.dataset.id;if(r&&await this.showConfirmDialog("確定要刪除此管理員帳號嗎？此操作無法復原。"))try{const{DatabaseService:a}=await Promise.resolve().then(()=>v);await a.deleteAdminUser(r),await this.showAlertDialog("管理員帳號已刪除"),await this.updatePageContent()}catch(a){console.error("Failed to delete admin user:",a),await this.showAlertDialog(`刪除失敗：${a instanceof Error?a.message:"未知錯誤"}`)}})})}async renderDashboard(){let e=[],t=[],o="連接失敗";try{const[n,s]=await Promise.all([fetch("http://localhost:3002/conversations").catch(()=>null),fetch("http://localhost:3002/manual-indexes").catch(()=>null)]);n!=null&&n.ok&&(e=await n.json(),o="正常連接"),s!=null&&s.ok&&(t=await s.json())}catch(n){console.error("Failed to load dashboard data:",n)}return`
          ...
          <div style="margin-bottom: 24px; padding-bottom: 24px; border-bottom: 1px solid #e5e7eb;">
            ...
            <div style="margin-bottom: 16px;">
              ...
              <input
                ...
                name="llmDeployment"
                placeholder="gpt-4"
                value="${((r=e.azureOpenAI)==null?void 0:r.deployment)||((l=e.llmAPI)==null?void 0:l.deployment)||""}"
                style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; box-sizing: border-box; background: white; color: #1f2937;"
              />
            </div>
          </div>
          ...
          <div style="margin-bottom: 24px;">
            ...
            <div style="margin-bottom: 16px;">
              ...
              <input
                ...
                name="embedEndpoint"
                placeholder="https://your-resource.openai.azure.com/"
                value="${((a=e.embeddingAPI)==null?void 0:a.endpoint)||((c=e.azureOpenAI)==null?void 0:c.endpoint)||""}"
                style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; box-sizing: border-box; background: white; color: #1f2937;"
              />
            </div>
            ...
            <div style="margin-bottom: 16px;">
              ...
              <input
                ...
                name="embedApiKey"
                placeholder="your-api-key"
                value="${((d=e.embeddingAPI)==null?void 0:d.apiKey)||((p=e.azureOpenAI)==null?void 0:p.apiKey)||""}"
                style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; box-sizing: border-box; background: white; color: #1f2937;"
              />
            </div>
    ...
    `,document.body.appendChild(o);const n=o.querySelector("#edit-index-form"),s=o.querySelector("#cancel-edit-btn");n.addEventListener("submit",async i=>{i.preventDefault();const r=o.querySelector("#edit-index-name").value,l=o.querySelector("#edit-index-description").value,a=o.querySelector("#edit-index-content").value;if(!r||!a){await this.showAlertDialog("請填寫名稱和內容");return}try{await C.update(e,{title:r,description:l,content:a,url:""}),await this.showAlertDialog("索引已更新"),document.body.removeChild(o),await this.updatePageContent()}catch(c){await this.showAlertDialog(`更新失敗：${c instanceof Error?c.message:"未知錯誤"}`)}}),s.addEventListener("click",()=>{document.body.removeChild(o)}),o.addEventListener("click",i=>{i.target===o&&document.body.removeChild(o)})}async showAddIndexModal(){const e=document.createElement("div");e.style.cssText=`
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
    ...
    `,document.body.appendChild(e);const t=e.querySelector("#add-index-form"),o=e.querySelector("#cancel-add-btn");t.addEventListener("submit",async n=>{n.preventDefault();const s=e.querySelector("#add-index-name").value,i=e.querySelector("#add-index-description").value,r=e.querySelector("#add-index-url").value,l=e.querySelector("#add-index-content").value;if(!s||!l){await this.showAlertDialog("請填寫名稱和內容");return}try{await C.create({title:s,description:i,content:l,url:r||void 0}),await this.showAlertDialog("索引已新增"),document.body.removeChild(e),await this.updatePageContent()}catch(a){await this.showAlertDialog(`新增失敗：${a instanceof Error?a.message:"未知錯誤"}`)}}),o.addEventListener("click",()=>{document.body.removeChild(e)}),e.addEventListener("click",n=>{n.target===e&&document.body.removeChild(e)})}async showDeleteConfirmDialog(e){const t=await C.getById(e);if(!t){await this.showAlertDialog("找不到該索引");return}if(await this.showConfirmDialog(`確定要刪除索引「${t.title||t.name||"未命名"}」嗎？此操作無法復原。`))try{await C.delete(e),await this.showAlertDialog("索引已刪除"),await this.updatePageContent()}catch(n){await this.showAlertDialog(`刪除失敗：${n instanceof Error?n.message:"未知錯誤"}`)}}async renderConversations(){try{const{CustomerServiceManager:e}=await Promise.resolve().then(()=>L),t=await e.getAllConversations();return`
          ...
          `:`
            <div style="overflow-x: auto;">
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                  ${t.slice().reverse().map(o=>{const n=o.conversation_id||o.conversationId||o.id,s=o.user_id||o.userId||"undefined",i=Array.isArray(o.messages)?o.messages:[],r=o.status||"active",l=o.created_at||o.createdAt||o.startedAt;return`
                    <tr style="border-bottom: 1px solid #f1f5f9;">
                      ...
                      <td style="padding: 16px; color: #6b7280; font-size: 14px;">${new Date(l).toLocaleString()}</td>
                      <td style="padding: 16px;">
                        <div style="display: flex; gap: 8px;">
      ...
      `}}async renderAdminUsers(){try{const{AdminUserManager:e}=await Promise.resolve().then(()=>N),t=await e.getAllAdminUsers();return`
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
      ...
      `}}async renderSystemSettings(){let e={},t=[];try{const{DatabaseService:i}=await Promise.resolve().then(()=>v),[r,l]=await Promise.all([i.getSettings().catch(()=>({})),i.getAdminUsers().catch(()=>[])]);e=r,t=l}catch(i){console.error("Failed to load system settings:",i)}const o=e.default_reply||"",n=e.system_prompt||"",s=e.llms_txt_url||"";return`
    ...
    `,document.body.appendChild(e);const t=e.querySelector("#add-admin-user-form"),o=e.querySelector("#cancel-add-admin-btn");t.addEventListener("submit",async n=>{n.preventDefault();const s=e.querySelector("#add-admin-username").value,i=e.querySelector("#add-admin-password").value,r=e.querySelector("#add-admin-email").value;try{const{DatabaseService:l}=await Promise.resolve().then(()=>v);await l.createAdminUser(s,i,r),document.body.removeChild(e),await this.showAlertDialog("管理員帳號已新增"),await this.updatePageContent()}catch(l){await this.showAlertDialog(`新增失敗：${l instanceof Error?l.message:"未知錯誤"}`)}}),o.addEventListener("click",()=>{document.body.removeChild(e)}),e.addEventListener("click",n=>{n.target===e&&document.body.removeChild(e)})}async showConversationModal(e){try{const{CustomerServiceManager:t}=await Promise.resolve().then(()=>L),o=await t.getConversationById(e);if(!o){await this.showAlertDialog("找不到該對話記錄");return}const n=o.conversation_id||o.conversationId||o.id,s=o.user_id||o.userId||"undefined";let i=[];if(typeof o.messages=="string")try{i=JSON.parse(o.messages)}catch(b){console.error("Failed to parse messages:",b),i=[]}else Array.isArray(o.messages)&&(i=o.messages);const r=o.status||"active",l=o.created_at||o.createdAt,a=o.updated_at||o.updatedAt,c=document.createElement("div");c.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
        ...
        ">
          ...
          <div style="margin-bottom: 16px; padding: 16px; background: #f9fafb; border-radius: 8px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; font-size: 14px;">
              ...
              <div><strong>建立時間:</strong> ${l?new Date(l).toLocaleString("zh-TW"):"未知"}</div>
              <div><strong>更新時間:</strong> ${a?new Date(a).toLocaleString("zh-TW"):"未知"}</div>
            </div>
          </div>
          ...
          <div style="max-height: 400px; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
            ...
            ${i.length>0?i.map(b=>`
                <div style="margin-bottom: 12px; padding: 12px; border-radius: 8px; ${b.role==="user"?"background: #eff6ff; margin-left: 20px;":"background: #f0fdf4; margin-right: 20px;"}">
                  <div style="font-weight: 600; color: #374151; margin-bottom: 4px;">
                    ${b.role==="user"?"👤 用戶":"🤖 助理"}
                    <span style="font-weight: normal; color: #6b7280; font-size: 12px; margin-left: 8px;">
                      ${b.timestamp?new Date(b.timestamp).toLocaleString("zh-TW"):""}
                    </span>
                  </div>
                  <div style="color: #1f2937; line-height: 1.5;">${b.content||""}</div>
                </div>
      ...
      `,document.body.appendChild(c);const d=c.querySelector("#close-conversation-modal"),p=c.querySelector("#close-conversation-modal-btn"),h=c.querySelector("#send-customer-service-reply"),u=c.querySelector("#customer-service-reply"),g=()=>{document.body.removeChild(c)};d==null||d.addEventListener("click",g),p==null||p.addEventListener("click",g),h==null||h.addEventListener("click",async()=>{const b=u==null?void 0:u.value.trim();if(!b){await this.showAlertDialog("請輸入回覆內容");return}try{const{CustomerServiceManager:y}=await Promise.resolve().then(()=>L);await y.addCustomerServiceReply(e,b,"客服")?(await this.showAlertDialog("回覆已發送"),g(),await this.updatePageContent()):await this.showAlertDialog("發送失敗，請稍後再試")}catch(y){console.error("Failed to send reply:",y),await this.showAlertDialog(`發送失敗：${y instanceof Error?y.message:"未知錯誤"}`)}}),c.addEventListener("click",b=>{b.target===c&&g()})}catch(t){console.error("Error showing conversation modal:",t),await this.showAlertDialog("載入對話詳情失敗")}}}class U{constructor(){m(this,"config");m(this,"panel");m(this,"conversationState");m(this,"initialized",!1);m(this,"captureMode",!1);m(this,"adminPanel");m(this,"floatingIcon");m(this,"screenshotMode",!1);m(this,"hoverHandler",null);m(this,"mouseLeaveHandler",null)}async loadRulesFromSQL(){try{const e=await fetch("http://localhost:3002/rules");if(!e.ok)return console.log("No rules found in database, using empty array"),[];const t=await e.json();return Array.isArray(t)?t:[]}catch(e){return console.error("Failed to load rules from SQL:",e),[]}}async init(e){var o,n,s;if(this.initialized){console.warn("ServiceModuler already initialized");return}this.config=e,console.log("✅ Widget initializing");const t=e.telegram&&e.telegram.botToken&&e.telegram.chatId?e.telegram:void 0;window.SM_TELEGRAM_CONFIG=t,await this.loadRulesFromSQL(),this.panel=new w(((o=e.ui)==null?void 0:o.width)||"33.33%",((n=e.ui)==null?void 0:n.position)||"right"),this.panel.setCallbacks({onSendMessage:(i,r)=>this.handleSendMessage(i,r),onSelectRule:i=>this.handleSelectRule(i),onClose:()=>this.handleClose(),onOpen:()=>this.handleOpen()}),await this.loadConversationState(),this.adminPanel||(this.adminPanel=new F),window.location.pathname==="/lens-service"&&this.openAdminPanel(),this.bindGlobalKeyboardShortcuts(),((s=e.ui)==null?void 0:s.iconPosition)!==!1&&!this.isAdminPage()&&this.createFloatingIcon(),this.initialized=!0,e.debug&&console.log("ServiceModuler initialized",e)}bindGlobalKeyboardShortcuts(){document.addEventListener("keydown",e=>{var t,o;e.key&&e.key.toLowerCase()==="q"&&((t=this.panel)!=null&&t.isPanelOpen())?(console.log("🎯 Q key pressed, panel is open, enabling screenshot mode"),this.enableScreenshotMode()):e.key&&e.key.toLowerCase()==="q"&&console.log("🎯 Q key pressed, but panel is not open:",(o=this.panel)==null?void 0:o.isPanelOpen())}),document.addEventListener("keyup",e=>{e.key&&e.key.toLowerCase()==="q"&&this.disableScreenshotMode()}),document.addEventListener("click",e=>{var t;this.screenshotMode&&((t=this.panel)!=null&&t.isPanelOpen())&&(console.log("📸 Screenshot click detected"),e.preventDefault(),e.stopPropagation(),this.captureElementScreenshot(e.target))},!0)}open(){var e;if(!this.initialized){console.error("ServiceModuler not initialized. Call init() first.");return}(e=this.panel)==null||e.open()}close(){var e;(e=this.panel)==null||e.close()}async sendMessage(e,t){var n,s,i;if(!this.initialized||!this.panel){console.error("ServiceModuler not initialized");return}const o={role:"user",content:e||"請分析這張圖片",timestamp:Date.now()};(n=this.conversationState)==null||n.messages.push(o),this.panel.addMessage(o),this.saveConversationState();try{let r,l,a=!1;const c=((s=this.conversationState)==null?void 0:s.sessionId)||this.generateSessionId(),d=localStorage.getItem("lens_service_user_id")||"default_user";if(t)r=await this.processImageMessage(e,t);else{const h=await this.processTextMessage(e,c,d);r=h.response,l=h.sources,a=h.needsHumanReply,a&&await this.sendTelegramNotification(e,c)}const p={role:"assistant",content:r,timestamp:Date.now(),sources:l};(i=this.conversationState)==null||i.messages.push(p),this.panel.addMessage(p),this.saveConversationState(),await this.saveConversationToDatabase(c,d)}catch(r){console.error("Error processing message:",r);const l={role:"assistant",content:`抱歉，發生錯誤：${r instanceof Error?r.message:"未知錯誤"}`,timestamp:Date.now()};this.panel.addMessage(l)}}async processTextMessage(e,t,o){var n,s,i,r;try{const{DatabaseService:l}=await Promise.resolve().then(()=>v);await l.initializePool();const a=await l.getSetting("system_prompt")||"你是一個專業的客服助手，請用繁體中文回答問題。",c=await l.getSetting("default_reply")||"很抱歉，我無法回答這個問題。請聯繫人工客服獲得更多幫助。",{ManualIndexService:d}=await Promise.resolve().then(()=>H),p=await d.search(e);console.log("🔍 Manual index search results:",p);const{LlmsTxtService:h}=await Promise.resolve().then(()=>K),u=await h.searchChunks(e);console.log("🔍 LLMs.txt search results:",u);const g=[...p.map(x=>({type:"manual_index",title:x.title||x.name,content:x.content,description:x.description||""})),...u.map(x=>({type:"llms_txt",title:"LLMs.txt",content:x.context,score:x.score}))];if(g.length===0)return console.log("❌ No relevant content found, using default reply"),{response:c,sources:[],needsHumanReply:!0};if(!((s=(n=this.config)==null?void 0:n.azureOpenAI)!=null&&s.endpoint)||!((r=(i=this.config)==null?void 0:i.azureOpenAI)!=null&&r.apiKey))return console.warn("Azure OpenAI not configured, using default reply"),{response:c,sources:[],needsHumanReply:!0};const b=g.map(x=>x.type==="manual_index"?`【手動索引】
標題：${x.title}
