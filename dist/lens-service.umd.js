...
`).map(y=>y.trim()).filter(y=>y.length>0);this.saveIPWhitelist(b),alert(`已更新 IP 白名單（${b.length} 個 IP）`);const v=this.container.querySelector("#admin-content");v&&(v.innerHTML=this.renderPageContent(),this.bindEvents())});const r=this.container.querySelector("#add-manual-index-form");r&&r.addEventListener("submit",p=>{p.preventDefault(),p.stopPropagation();const h=this.container.querySelector("#index-name"),u=this.container.querySelector("#index-description"),b=this.container.querySelector("#index-content"),v=(h==null?void 0:h.value)||"",y=(u==null?void 0:u.value)||"",C=(b==null?void 0:b.value)||"";if(!v||!C){alert("請填寫名稱和內容");return}try{A.create({name:v,description:y,content:C}),alert("索引已新增");const I=this.container.querySelector("#admin-content");I&&(I.innerHTML=this.renderPageContent(),this.bindEvents())}catch(I){alert(`新增失敗：${I instanceof Error?I.message:"未知錯誤"}`)}}),this.container.querySelectorAll(".delete-index-btn").forEach(p=>{p.addEventListener("click",()=>{const h=p.dataset.id;if(h&&confirm("確定要刪除這個索引嗎？"))try{A.delete(h),alert("索引已刪除");const u=this.container.querySelector("#admin-content");u&&(u.innerHTML=this.renderPageContent(),this.bindEvents())}catch(u){alert(`刪除失敗：${u instanceof Error?u.message:"未知錯誤"}`)}})});const a=this.container.querySelector("#api-config-form");a&&a.addEventListener("submit",p=>{var R,N,$,q,L,M;p.preventDefault(),p.stopPropagation();const h=((R=this.container.querySelector("#llm-endpoint"))==null?void 0:R.value)||"",u=((N=this.container.querySelector("#llm-api-key"))==null?void 0:N.value)||"",b=(($=this.container.querySelector("#llm-deployment"))==null?void 0:$.value)||"",v=((q=this.container.querySelector("#embed-endpoint"))==null?void 0:q.value)||"",y=((L=this.container.querySelector("#embed-api-key"))==null?void 0:L.value)||"",C=((M=this.container.querySelector("#embed-deployment"))==null?void 0:M.value)||"",I={azureOpenAI:{endpoint:h,apiKey:u,deployment:b,embeddingDeployment:C},llmAPI:{endpoint:h,apiKey:u,deployment:b},embeddingAPI:{endpoint:v,apiKey:y,deployment:C}};m.saveConfig(I),alert("API 設定已儲存")});const d=this.container.querySelector("#agent-tool-config-form");d&&d.addEventListener("submit",p=>{var v,y;p.preventDefault(),p.stopPropagation();const h=((v=this.container.querySelector("#manual-index-enabled"))==null?void 0:v.checked)||!1,u=((y=this.container.querySelector("#frontend-pages-enabled"))==null?void 0:y.checked)||!1,b=m.loadAgentToolConfig();if(b){b.manualIndex.enabled=h,b.frontendPages.enabled=u,m.saveAgentToolConfig(b),alert("Agent 設定已儲存");const C=this.container.querySelector("#admin-content");C&&(C.innerHTML=this.renderPageContent(),this.bindEvents())}});const c=this.container.querySelector("#sql-plugin-config-form");c&&c.addEventListener("submit",p=>{var q,L,M,K,U,F,Y,Q;p.preventDefault(),p.stopPropagation();const h=((q=this.container.querySelector("#sql-plugin-enabled"))==null?void 0:q.checked)||!1,u=parseInt(((L=this.container.querySelector("#sql-plugin-priority"))==null?void 0:L.value)||"5"),b=((M=this.container.querySelector("#sql-api-endpoint"))==null?void 0:M.value)||"",v=((K=this.container.querySelector("#sql-connection-id"))==null?void 0:K.value)||"",y=((U=this.container.querySelector("#sql-search-table"))==null?void 0:U.value)||"knowledge_base",C=((F=this.container.querySelector("#sql-title-column"))==null?void 0:F.value)||"title",I=((Y=this.container.querySelector("#sql-content-column"))==null?void 0:Y.value)||"content",R=((Q=this.container.querySelector("#sql-url-column"))==null?void 0:Q.value)||"url",N={enabled:h,priority:u,apiEndpoint:b,connectionId:v,searchTable:y,titleColumn:C,contentColumn:I,urlColumn:R};localStorage.setItem("sm_sql_plugin_config",JSON.stringify(N)),alert("SQL Plugin 設定已儲存");const $=this.container.querySelector("#admin-content");$&&($.innerHTML=this.renderPageContent(),this.bindEvents())});const g=this.container.querySelector("#sql-connection-form");g&&g.addEventListener("submit",p=>{var b,v;p.preventDefault(),p.stopPropagation();const h=((b=this.container.querySelector("#sql-conn-name"))==null?void 0:b.value)||"",u=(v=this.container.querySelector("#sql-conn-type"))==null?void 0:v.value;if(!h){alert("請輸入連接名稱");return}try{z.create({name:h,type:u,host:"localhost",port:3306,database:"mydb",username:"user",password:"password",queryTemplate:"SELECT * FROM {table} WHERE {conditions}",resultMapping:{titleField:"title",contentField:"content",urlField:"url"}}),alert("SQL 連接已新增");const y=this.container.querySelector("#admin-content");y&&(y.innerHTML=this.renderPageContent(),this.bindEvents())}catch(y){console.error("Error creating SQL connection:",y),alert("新增失敗")}}),this.container.querySelectorAll(".delete-sql-connection").forEach(p=>{p.addEventListener("click",()=>{const h=p.dataset.id;if(h&&confirm("確定要刪除這個連接嗎？"))try{z.delete(h),alert("連接已刪除");const u=this.container.querySelector("#admin-content");u&&(u.innerHTML=this.renderPageContent(),this.bindEvents())}catch(u){console.error("Error deleting SQL connection:",u),alert("刪除失敗")}})})}renderAdminUI(){return`
    ...
    `}renderAgentAndAPI(){var n,o,i,r,s,a,d,c,g,S,p,h;const e=m.loadConfig()||{},t=m.loadAgentToolConfig();return`
          ...
          <div style="margin-bottom: 24px;">
            ...
            <div style="margin-bottom: 16px;">
              ...
              <input
                ...
                name="embedApiKey"
                placeholder="your-api-key"
                value="${((g=e.embeddingAPI)==null?void 0:g.apiKey)||((S=e.azureOpenAI)==null?void 0:S.apiKey)||""}"
                style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; box-sizing: border-box; background: white; color: #1f2937;"
              />
            </div>
