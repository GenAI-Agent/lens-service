...
class z {
  ...
  static async search(e) {
    try {
      ...
      if (console.log("🔍 ManualIndexService.getAll() returned:", o.length, "indexes"), o.length > 0 && console.log("🔍 First index:", o[0]), !e.trim()) return o;
      const t = e.toLowerCase();
      console.log("🔍 Query (lowercase):", t);
      const n = o.filter((s) => {
        const i = (s.title || s.name || "").toLowerCase(), r = (s.description || "").toLowerCase(), a = (s.content || "").toLowerCase();
        console.log("🔍 Checking index:", { title: i, description: r.substring(0, 30), content: a.substring(0, 30) }), console.log("🔍 title.includes(queryLower):", i.includes(t)), console.log("🔍 description.includes(queryLower):", r.includes(t)), console.log("🔍 content.includes(queryLower):", a.includes(t));
        const d = i.includes(t) || r.includes(t) || a.includes(t);
        return console.log("🔍 Matches:", d), d && console.log("🔍 Match found:", { title: i, description: r.substring(0, 50) }), d;
      });
...
`).map((g) => g.trim()).filter((g) => g.length > 0);
    ...
    a && a.addEventListener("submit", async (l) => {
      var f, L, $, A, E, P, q, M;
      l.preventDefault(), l.stopPropagation();
      const c = ((f = this.container.querySelector("#sql-plugin-enabled")) == null ? void 0 : f.checked) || !1, p = parseInt(((L = this.container.querySelector("#sql-plugin-priority")) == null ? void 0 : L.value) || "5"), u = (($ = this.container.querySelector("#sql-api-endpoint")) == null ? void 0 : $.value) || "", g = ((A = this.container.querySelector("#sql-connection-id")) == null ? void 0 : A.value) || "", m = ((E = this.container.querySelector("#sql-search-table")) == null ? void 0 : E.value) || "knowledge_base", x = ((P = this.container.querySelector("#sql-title-column")) == null ? void 0 : P.value) || "title", w = ((q = this.container.querySelector("#sql-content-column")) == null ? void 0 : q.value) || "content", k = ((M = this.container.querySelector("#sql-url-column")) == null ? void 0 : M.value) || "url", I = {
        enabled: c,
        priority: p,
        apiEndpoint: u,
        connectionId: g,
    ...
    e && e.addEventListener("click", async () => {
      ...
      n.addEventListener("click", async (s) => {
        ...
        if (i && await this.showConfirmDialog("確定要刪除這個對話嗎？此操作無法復原。"))
          try {
            const { CustomerServiceManager: a } = await import("./CustomerServiceManager-f8UYHN2x.mjs");
  ...
  async renderConversations() {
    try {
      const { CustomerServiceManager: e } = await import("./CustomerServiceManager-f8UYHN2x.mjs"), o = await e.getAllConversations();
      return `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
  ...
  async renderAdminUsers() {
    try {
      const { AdminUserManager: e } = await import("./AdminUserManager-Cev40dGr.mjs"), o = await e.getAllAdminUsers();
      return `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
  ...
  async showConversationModal(e) {
    try {
      const { CustomerServiceManager: o } = await import("./CustomerServiceManager-f8UYHN2x.mjs"), t = await o.getConversationById(e);
      if (!t) {
      ...
      l == null || l.addEventListener("click", g), c == null || c.addEventListener("click", g), p == null || p.addEventListener("click", async () => {
        ...
        try {
          const { CustomerServiceManager: x } = await import("./CustomerServiceManager-f8UYHN2x.mjs");
...
class F {
  ...
  async processTextMessage(e, o, t) {
    ...
    try {
      ...
      const { LlmsTxtService: p } = await import("./LlmsTxtService-CE--EXo5.mjs"), u = await p.searchChunks(e);
